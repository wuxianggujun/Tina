...
public:
        ...
        Tina::GameStartupContext&) override
    {
        return std::unique_ptr<Tina::IGameState>{
            std::make_unique<NullGameState>(m_targetFrameCount, m_counters)};
    }
...
{
    std::cerr << "{\"status\":\"error\",\"code\":{\"domain\":"
              << static_cast<std::uint16_t>(error.code.domain)
...
[[nodiscard]] Tina::EngineFactories createEngineFactories()
{
    return Tina::EngineFactories{
        .createMonotonicClock = []() -> Tina::Core::Result<
            std::unique_ptr<Tina::Core::IMonotonicClock>> {
...
[[nodiscard]] Tina::Core::Status verifyLifecycle(
    Tina::RunExitReason exitReason,
    u64 targetFrameCount,
    const LifecycleCounters& counters)
{
    ...
    if (counters.frameUpdates != targetFrameCount || counters.stateExits != 1
...
int main(int argumentCount, char** arguments)
{
    ...
    auto hostResult = Tina::EngineHost::Create(
        Tina::EngineConfig::Defaults(),
    ...
    std::cout << "{\"status\":\"ok\",\"sample\":\"tina_sample_null\",\"frames\":"
              << counters.frameUpdates
              << ",\"exit\":\"GameRequestedExitAfterCurrentFrame\",\"stateExits\":"
              << counters.stateExits

...
{
    // Stable product ids must match samples/2d_tilemap_bgfx/assets/sample_2d.recipe.
    ...
    auto recipePath = Tina::Core::applicationFilePath("assets/sample_2d.recipe");
    if (!recipePath)
    {
    ...
    auto catalogRoot = Tina::Sample::prepareApplicationContentDirectory("content");
    if (!catalogRoot)
    {
...
int main(int argc, char** argv)
{
    ...
    // The cooked catalog stays beside the executable; prepareCatalog wipes it before each cook,
    // so deleting it on exit only made a failed run harder to inspect.

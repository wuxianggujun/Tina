...
int runAuthoredSceneSample(int argumentCount, char** arguments)
{
    ...
    // Beside the executable rather than in %TEMP%: the authored .tworld and the catalog it
    // references are one run's output, and keeping them in the build tree makes a failed run
    // inspectable instead of leaving a stale sibling somewhere unrelated.
    auto preparedRoot = Tina::Sample::prepareApplicationContentDirectory("content");
    if (!preparedRoot)
    {
        printError(preparedRoot.error());
        return 1;
    }
    const std::filesystem::path root = std::move(*preparedRoot);

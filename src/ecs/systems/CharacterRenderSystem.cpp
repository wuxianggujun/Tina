...
bool CharacterRenderSystem::initialize(Tina::Renderer::ShaderManager& shaders)
{
    // 使用世界基础管线绘制简易 AABB（可后续替换为专用角色着色器）
    m_prog = Tina::Renderer::ShaderCatalog::Load(shaders, Tina::Renderer::ShaderCatalog::Tag::WorldSolid);
    m_layout.begin()
...
void CharacterRenderSystem::renderAABB(float x, float y, float width, float height,
    ...
    if (bgfx::allocTransientBuffers(&tvb, m_layout, 4, &tib, 6)) {
        // 复制数据

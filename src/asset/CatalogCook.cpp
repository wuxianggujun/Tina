    ...
    if (tokens[2] == "file")
    {
            ...
            !validated)
        {
            return Core::failure(std::move(validated.error()).withContext(
                "parseAudioClipInline", "validateAudioPath"));
        }
        std::pmr::unsynchronized_pool_resource audioMemory;
        auto bytes = Core::readFile(
            *path, Core::ReadFileConfig{.maxBytes = Detail::MaxAudioSourceFileBytes, .memoryResource = &audioMemory});
        if (!bytes)
        {
            return Core::failure(std::move(bytes.error()).withContext("parseAudioClipInline", "readAudio"));
        }

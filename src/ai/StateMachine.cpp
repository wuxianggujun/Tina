...
StateMachine::~StateMachine() noexcept {
    ...
    if (m_state == StateMachineState::Running && m_storage && m_storage->states[m_activeState].exit && m_blackboard)
...
void StateMachine::fault() noexcept {
    if (m_state == StateMachineState::Running && m_storage && m_blackboard) {
...
Core::Status StateMachine::cancel() {
    ...
    Detail::DispatchGuard guard{m_dispatching};
    if (m_state == StateMachineState::Running && m_storage && m_blackboard) { const auto& s=m_storage->states[m_activeState]; if(s.exit) s.exit(*m_blackboard,s.userData); }

...
// Texture2D upload vocabulary translation. Render publishes backend-neutral names, so
// the bgfx tokens live here; a missing case would be a compile error rather than a
...
[[nodiscard]] u64 toBgfxTextureFlags(const Texture2DUploadDesc& desc) noexcept
{
...
class BgfxRenderDevice final : public IRenderDevice {
  ...
  private:
    ...
    [[nodiscard]] Core::Result<GpuTextureId> createTexture2D(const Texture2DUploadDesc& desc) override
    {
        ...
        const u64 flags = toBgfxTextureFlags(desc);
        const bool hasMips = desc.levels.size() > 1U;
        // Ask bgfx whether the running renderer supports the format before creating:
        // a compressed format missing on this adapter otherwise surfaces as an invalid
        // handle with no indication that the format, not the pixels, was the problem.
        ...
        // bgfx takes the whole chain as one blob in level order, which is exactly how
        // the cooked payload stores it, so the levels are concatenated rather than
        // uploaded per level.
        usize totalBytes = 0;

...
#include "fs_tina_ui_textured_quad_glsl.bin.h"
#include "fs_tina_ui_textured_quad_spv.bin.h"
#include "vs_tina_ui_textured_quad_glsl.bin.h"
#include "vs_tina_ui_textured_quad_spv.bin.h"

#if BX_PLATFORM_WINDOWS
#include "fs_tina_ui_textured_quad_dxbc.bin.h"
#include "vs_tina_ui_textured_quad_dxbc.bin.h"
#endif
...
constexpr bgfx::EmbeddedShader EmbeddedShaders[] = {
    {
        "vs_tina_ui_textured_quad",
        {
#if BX_PLATFORM_WINDOWS
            {bgfx::RendererType::Direct3D11, vs_tina_ui_textured_quad_dxbc,
             sizeof(vs_tina_ui_textured_quad_dxbc)},
#endif
            {bgfx::RendererType::OpenGL, vs_tina_ui_textured_quad_glsl,
             sizeof(vs_tina_ui_textured_quad_glsl)},
#if defined(TINA_RENDER_BGFX_MOBILE_SHADERS)
    ...
    {
        "fs_tina_ui_textured_quad",
        {
#if BX_PLATFORM_WINDOWS
            {bgfx::RendererType::Direct3D11, fs_tina_ui_textured_quad_dxbc,
             sizeof(fs_tina_ui_textured_quad_dxbc)},
#endif
            {bgfx::RendererType::OpenGL, fs_tina_ui_textured_quad_glsl,
             sizeof(fs_tina_ui_textured_quad_glsl)},
#if defined(TINA_RENDER_BGFX_MOBILE_SHADERS)
...
[[nodiscard]] Core::Error unsupportedShaderError()
{
    Core::Error error{RenderErrorCode::DeviceInitializationFailed,
                      "The active bgfx renderer has no cooked Tina textured UI shader"};
    error.addContext("createUITexturedQuadProgram");
    return error;
}

} // namespace

Core::Result<bgfx::ProgramHandle> createUITexturedQuadProgram()
{
    const bgfx::RendererType::Enum renderer = bgfx::getRendererType();
    const bgfx::ShaderHandle vertexShader =
        bgfx::createEmbeddedShader(EmbeddedShaders, renderer, "vs_tina_ui_textured_quad");
    ...
    const bgfx::ShaderHandle fragmentShader =
        bgfx::createEmbeddedShader(EmbeddedShaders, renderer, "fs_tina_ui_textured_quad");
    ...
    if (!bgfx::isValid(program))
    {
        return Core::failure(RenderErrorCode::DeviceInitializationFailed,
                             "bgfx rejected the Tina textured UI shader program");
    }
    return program;
}

...
// Private tag for the existing retained side-storage layout. Public authoring,
// hit, structure, semantics, and accessibility contracts use composition data
// and never expose this implementation detail.
...
{
    return value == UIAxisAlignment::Start || value == UIAxisAlignment::Center || value == UIAxisAlignment::End ||
           value == UIAxisAlignment::Stretch;
}

[[nodiscard]] bool isValidContentAlignment(UIContentAlignment alignment) noexcept
{
    const auto validAxis = [](UIAxisAlignment axis) noexcept {
        return axis == UIAxisAlignment::Start || axis == UIAxisAlignment::Center || axis == UIAxisAlignment::End;
    };
...
{
    return value == UIAlignSelf::Auto || value == UIAlignSelf::Start || value == UIAlignSelf::Center ||
           value == UIAlignSelf::End || value == UIAlignSelf::Stretch;
}
...
[[nodiscard]] UIAxisAlignment resolvedItemAlignment(UIAxisAlignment parentAlignment,
                                                    UIAlignSelf alignSelf) noexcept
{
    switch (alignSelf)
    {
...
[[nodiscard]] Core::Result<UILayoutStyle> normalizeLayoutStyle(UILayoutStyle style)
{
    ...
    if (Core::Status status = normalizeLayoutLength(style.flexItem.basis, true); !status)
    {
        return Core::failure(status.error());
    }
    if (Core::Status status = normalizeSpacing(style.flexContainer.gap.row); !status)
    {
        return Core::failure(status.error());
    }
    if (Core::Status status = normalizeSpacing(style.flexContainer.gap.column); !status)
    {
        return Core::failure(status.error());
    }
    if (Core::Status status = normalizeLayoutLength(style.overlay.offset.x, false); !status)
    {
        return Core::failure(status.error());
    }
    if (Core::Status status = normalizeLayoutLength(style.overlay.offset.y, false); !status)
    {
        return Core::failure(status.error());
    }
    if (!isValidFlexDirection(style.flexContainer.direction) ||
        !isValidJustifyContent(style.flexContainer.justifyContent) ||
        !isValidAxisAlignment(style.flexContainer.alignItems) || !isValidAlignSelf(style.flexItem.alignSelf) ||
        !isValidAxisAlignment(style.overlay.horizontal) || !isValidAxisAlignment(style.overlay.vertical) ||
        !isValidPlacement(style.placement) || !isValidVisibility(style.visibility))
    {
...
[[nodiscard]] Core::Result<NormalizedCapacityConfig> normalizeCapacity(UIContextCapacityConfig config)
{
    ...
    const usize canvasCommandCapacity =
        config.canvasCommandCapacity == 0 ? config.nodeCapacity : config.canvasCommandCapacity;
    ...
    return NormalizedCapacityConfig{
        ...
        .canvasCommandCapacity = canvasCommandCapacity,
...
struct UIContext::Impl final {
    ...
    [[nodiscard]] static bool isButtonChromeKind(BuiltinElementKind kind) noexcept
    {
        return kind == BuiltinElementKind::Button || kind == BuiltinElementKind::Dropdown || kind == BuiltinElementKind::DropdownItem ||
               kind == BuiltinElementKind::ListViewItem || kind == BuiltinElementKind::TreeViewItem;
    }

    [[nodiscard]] static bool isDefaultActivatableKind(BuiltinElementKind kind) noexcept
    {
        return kind == BuiltinElementKind::Button || kind == BuiltinElementKind::Dropdown || kind == BuiltinElementKind::DropdownItem ||
               kind == BuiltinElementKind::ListViewItem || kind == BuiltinElementKind::Checkbox ||
               kind == BuiltinElementKind::TreeViewItem || kind == BuiltinElementKind::RadioButton;
    }

    [[nodiscard]] static bool isKeyboardFocusableKind(BuiltinElementKind kind) noexcept
    {
        return isDefaultActivatableKind(kind) || kind == BuiltinElementKind::TextEdit || kind == BuiltinElementKind::ListView ||
                                                ...
                                                u32 activeModalEntryIndex) const noexcept
    {
        ...
        return entryIndex < entries.size() &&
               hasBehavior(entries[entryIndex].behaviors, UIElementBehavior::Focusable);
    }
    ...
    [[nodiscard]] static bool supportsWidgetText(BuiltinElementKind kind) noexcept
    {
        return kind == BuiltinElementKind::Label || kind == BuiltinElementKind::Button || kind == BuiltinElementKind::TextEdit ||
               kind == BuiltinElementKind::RadioButton || kind == BuiltinElementKind::Dropdown ||
               kind == BuiltinElementKind::DropdownItem || kind == BuiltinElementKind::ListViewItem ||
               kind == BuiltinElementKind::TreeViewItem;
    }

    [[nodiscard]] static UIContentAlignment defaultContentAlignment(BuiltinElementKind kind) noexcept
    {
        if (kind == BuiltinElementKind::Button)
        {
            return UIContentAlignment{
                .horizontal = UIAxisAlignment::Center,
                .vertical = UIAxisAlignment::Center,
            };
        }
        if (kind == BuiltinElementKind::TextEdit || kind == BuiltinElementKind::Dropdown ||
            kind == BuiltinElementKind::DropdownItem || kind == BuiltinElementKind::RadioButton ||
            kind == BuiltinElementKind::ListViewItem || kind == BuiltinElementKind::TreeViewItem)
        {
            return UIContentAlignment{
...
bool UIElementBuildTransaction::isActive() const noexcept
{
    ...
    const std::scoped_lock lock(lifetime->mutex);
    return lifetime->context != nullptr;
}

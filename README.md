...
M10-A2c–A2o provide cooked load, dependency order, batch load, package validate,
`openCatalogPackage`, the `tina_catalog_validate` CLI (including `--plan-loads`/`--load-assets`),
package summary diagnostics, load plans, one-shot package load, and shared pipeline e2e fixtures.
Handle/Lease, async IO, and Cooker remain out of scope.

...
optional miniaudio/Box2D, and a custom vNext retained-mode UI (`Tina::UI`). Game-facing contracts
expose no third-party or bgfx types. The Legacy `Tina.exe` / `src/ui` product graph has been
retired; product entry points are `tina_sample_*` under vNext Desktop bootstrap.
...
M10-A2c–A2q provide the sync catalog/cooked pipeline. M10-A3–A30 add CPU Handle/Lease, AssetSystem
load/request/pump with optional auto Null GPU upload to ReadyGpu, openAndBindCatalog, kind lookup,
typed Texture/Sprite/Tileset/TileMap payloads with inline recipe cooking, runtime `TileMapInstance`,
`IGridCollisionProvider`, visible `TileChunkView` extraction, tile-chunk→`RenderSprite2DInput` emit,
Sprite→RenderSprite UV bridge into RenderScene, bgfx Sprite2D `s_tex` sampling, GPU Texture2D
upload/bind SPI, `tina_sample_2d_catalog`, retirement ledger, bounded IO + optional CPU Task workers
with `TaskGroup`, Desktop default BoundedTaskSystem, Null UploadTicket ledger, `tina_sample_asset`,
cooked writers, atomic publish, and `tina_assetc` typed2d/recipe cooker. Full formal `tina_sample_2d`
(Box2D/UI product gate) and full cgltf Cooker remain out of scope.

...
M10-A2c–A2q provide the sync catalog/cooked pipeline. M10-A3–A13 add CPU Handle/Lease, AssetSystem
...
`tina_sample_asset` (including `--catalog=`), cooked/manifest wire writers, atomic
`writeFile`/`publishCatalogPackage`, and a minimal `tina_assetc` fixture cooker. CPU TaskGroup pools,
real bgfx GPU resources, and full cgltf Cooker remain out of scope.

...
M10-A2c–A2q provide the sync catalog/cooked pipeline. M10-A3–A29 add CPU Handle/Lease, AssetSystem
...
`IGridCollisionProvider`, visible `TileChunkView` extraction, Sprite→RenderSprite UV bridge into
RenderScene, bgfx Sprite2D `s_tex` sampling, GPU Texture2D upload/bind SPI, `tina_sample_2d_catalog`,
retirement ledger, bounded IO + optional CPU Task workers with `TaskGroup`, Desktop default
BoundedTaskSystem, Null UploadTicket ledger, `tina_sample_asset`, cooked writers, atomic publish, and
`tina_assetc` typed2d/recipe cooker. TileChunkRenderPacket/full formal `tina_sample_2d` and full

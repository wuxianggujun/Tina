...
M7-B1 private WindowSurface handoff, M7-B2 Desktop bootstrap plus real-GPU smoke, and the M7-C1b/M7-C1c-a
standalone retained tree, Flex-lite layout, and committed hit-snapshot foundations are complete. The private `tina_platform_glfw` backend now
...
CMake 4.2.3 Debug and Release builds pass with direct GTest 183/183, UI 54/54, GLFW 22/22, bgfx 11/11, and real
D3D11 Intel Iris Xe 300-frame smoke. Linux GCC 13.4 passes UI 54/54 plus the same 183/22/11 direct-test matrix
and a 300-frame Desktop run; Clang 22.1.8 passes UI 54/54 and that matrix under ASan/UBSan/LSan with no
...
`commitLayout()` publishes none of the structure/layout/hit candidates. This is snapshot data only: point hit-testing,
reverse-order target selection, Capture -> Target -> Bubble routing, Focus/Capture/Modal, Button interaction,

...
M10-A2c–A2q provide the sync catalog/cooked pipeline. M10-A3–A12 add CPU Handle/Lease, AssetSystem
load/request/pump with optional auto Null GPU upload to ReadyGpu, bounded IO Task + Main completion,
Desktop default BoundedTaskSystem, Null UploadTicket ledger, `tina_sample_asset`, cooked/manifest
wire writers, atomic `writeFile`/`publishCatalogPackage`, and a minimal `tina_assetc` fixture cooker.
CPU TaskGroup pools, real bgfx GPU resources, and full cgltf Cooker remain out of scope.

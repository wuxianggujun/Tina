...
M10-A2c–A2q provide the sync catalog/cooked pipeline. M10-A3–A22 add CPU Handle/Lease, AssetSystem
...
Sprite2D `s_tex` sampling with a default white texture, retirement ledger, bounded IO Task + Main
completion, Desktop default BoundedTaskSystem, Null UploadTicket ledger, `tina_sample_asset`, cooked
writers, atomic publish, `tina_assetc` typed2d/recipe cooker, and Texture2D/Sprite payload codecs.
Catalog→GPU texture upload, CPU TaskGroup pools, and full cgltf Cooker remain out of scope.

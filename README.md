...
M10-A2c–A2p provide cooked load, dependency order, batch load, package validate,
`openCatalogPackage`, the `tina_catalog_validate` CLI (including `--plan-loads`/`--load-assets`),
package summary diagnostics, load plans, one-shot package load, shared pipeline e2e fixtures, and
`totalCookedFileBytes`. Handle/Lease, async IO, and Cooker remain out of scope.

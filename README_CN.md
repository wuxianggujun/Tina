...
M10-A2c～A2q 同步 Catalog/Cooked 闭环；M10-A3～A15 Handle/Lease、AssetSystem、有界 IO Task、
Desktop BoundedTask、Null UploadTicket、ReadyCpu→ReadyGpu、retirement ledger、`tina_sample_asset`、
wire writer、原子 publish、`tina_assetc`（fixture + recipe）、Texture2D/Sprite payload v1。
仍不包含 CPU TaskGroup、bgfx 真实 GPU 采样绘制或 cgltf。

...
M10-A2c～A2q 同步 Catalog/Cooked 闭环；M10-A3～A24 Handle/Lease、AssetSystem、typed Texture/Sprite
cook/load/validate、Sprite→RenderScene、bgfx `s_tex` + GPU Texture2D 上传/绑定、
`tina_sample_2d_catalog` 可见样例、有界 IO Task、Null UploadTicket、ReadyCpu→ReadyGpu、
retirement ledger、`tina_sample_asset`/`tina_assetc`。仍不包含完整正式 tina_sample_2d/CPU TaskGroup/cgltf。

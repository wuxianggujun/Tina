...
M10-A2c～A2f 已实现：Cooked 加载、依赖序、批量同步加载，以及有界
`validateCatalogPackageOnDisk` 磁盘包校验。Windows MSVC Debug/Release `tina_asset_tests` 均34/34。
仍不包含 Handle/Lease、async IO、catalog 校验 CLI 或 Cooker。

...
M10-A2c～A2q 同步 Catalog/Cooked 闭环；M10-A3～A28 Handle/Lease、AssetSystem、typed Texture/Sprite/
Tileset/TileMap payload 与 recipe、TileMapInstance（edit/solid query）、Sprite→RenderScene、
bgfx GPU 纹理上传绘制、`tina_sample_2d_catalog`、有界 IO+CPU Task/TaskGroup、Null UploadTicket、
ReadyCpu→ReadyGpu、retirement ledger、`tina_sample_asset`/`tina_assetc`。仍不包含 chunk
extraction/完整正式 tina_sample_2d（Box2D）/cgltf。

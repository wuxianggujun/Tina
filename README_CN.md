...
M10-A2c～A2q 同步 Catalog/Cooked 闭环；M10-A3 CPU Handle/Lease；M10-A4 同步 `AssetSystem`；
M10-A5 主线程 `request`/`pump` 与 Queued/Loading/Failed。Windows `tina_asset_tests` 见最近门禁。
仍不包含 Task worker 线程池、GPU UploadTicket/retirement 或 Cooker。

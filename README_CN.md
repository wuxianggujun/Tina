...
M10-A2c～A2q 同步 Catalog/Cooked 闭环；M10-A3～A19 Handle/Lease、AssetSystem（openAndBind/按类型查找/
typed views/typed 校验）、有界 IO Task、Desktop BoundedTask、Null UploadTicket、ReadyCpu→ReadyGpu、
retirement ledger、`tina_sample_asset`、`tina_assetc`（typed2d/recipe/内联 texture2d·sprite）、
Texture2D/Sprite payload 与校验门禁。仍不包含 CPU TaskGroup、bgfx 真实纹理采样绘制或 cgltf。

...
M10-A2c～A2q 同步 Catalog/Cooked 闭环；M10-A3～A20 Handle/Lease、AssetSystem、typed Texture/Sprite
cook/load/validate、Sprite→RenderSprite UV 桥接、有界 IO Task、Null UploadTicket、ReadyCpu→ReadyGpu、
retirement ledger、`tina_sample_asset`/`tina_assetc`。仍不包含 CPU TaskGroup、bgfx 真实纹理采样或 cgltf。

...
| Theme/Style（A/B/C1 + UI-STYLE-001 slice） | `UITheme` token + `UIStyleRoleId` recipe + 属性 override mask/reset；startup-only 强类型 StyleClass、node-local pseudo-state 与 literal BoxFill stylesheet 已落地；**无**圆角子树 clip/毛玻璃/完整 CSS 或 token declaration |
...
stylesheet token/declaration 与 BoxFill 之外的属性面仍属后续扩展。
...
| 自定义 Theme/外观 | 部分可用 | 可替换 `UITheme`、选择封闭 `UIStyleRoleId`、做属性级 override，并在首个 retained node 前注册强类型 StyleClass、安装 node-local pseudo-state + literal BoxFill rule；尚无 token/declaration 与通用 selector |
...
逐角半径、圆角子树 clip、毛玻璃、stylesheet token/declaration 与完整 CSS 仍未实现；startup-only literal
BoxFill rule、node-local state 和 Runtime 注册入口已经可用。
...
| `UI-STYLE-001` | InProgress；强类型 StyleClass、node-local pseudo-state selector、literal BoxFill rule、预编译有界 stylesheet、Runtime startup facade 与固定 workload 已落地；待 token/declaration、reverse dependency 与更多属性面；不做完整 CSS |
...
Component/Behavior 与 StyleClass/pseudo-state 切片均已汇合，下一条 UI lane 是 Style token/declaration、
Image tint/opacity 等属性面，随后再做 paint-only Motion。不再重复列已删除的

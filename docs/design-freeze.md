...
| JSON 写出 | 自研仅写出的 `Core::JsonWriter`；不引入任何第三方 JSON 库，也不实现 JSON 解析器或 DOM（C++ 侧唯一的 JSON 解析仍是 cgltf 内置 jsmn）；数值格式化委托被包装的 `std::ostream` 故迁移后逐字节一致；转义统一到「完整转义」版（控制字节写 `\u00xx`，非法 UTF-8 原样透传） | [0038](adr/0038-json-writer-without-json-library.md) | Implemented：header-only、全 `noexcept`、无分配无异常无虚函数，13 份不一致的 `writeJsonString` 副本与 66 个调用点已一次性迁移并删除，迁移范围覆盖全部手写 JSON 发射点而非仅用过该 helper 的文件。gate 的按字节选行契约（`status` 第一、`sample` 第二紧邻、冒号后无空格、恰好一行 JSON）不变；对原先丢弃控制字节的 5 个样例是一次有意的行为变更 |
| 日志 | 前端按 `TINA_LOG_LEVEL_COMPILED` 编译期剥离（语句整体消失、参数不求值，`TINA_LOG_CRITICAL` 永不剥离）；`LogRecord` 自持 256 字节内联缓冲而非借用视图；队列有界且满时丢弃最新并计数；Error 及以上在 `write()` 返回前完成投递 | [0039](adr/0039-logging-frontend-and-async-sinks.md) | Implemented：不引入 spdlog/fmt，自研 `LogFormat.hpp` 的 `{}` 占位符（公共头不得命名 libc++ 19 缺失的设施，故排除 `std::format`）；宏不用 `__VA_OPT__`（MSVC 传统预处理器需 `/Zc:preprocessor`，公共头不能要求消费者传）；计数器 relaxed atomic、递归守卫 `thread_local`、sink 在 `m_sinkMutex` 下串行；默认同步，EngineHost 显式选 1024 槽异步。同步 flush 门槛由 `src/`+`include/` 的 125 处 `std::terminate()` 决定 |

...
| Network | 自研传输，不引入 asio/libuv/curl；owner-thread readiness 多路复用；TLS 是可选独立模块且 mbedTLS 类型不出现在公开头 | [0033](adr/0033-network-module-boundaries.md) | Implemented：UDP、TCP 客户端/listener、`IByteStream`、HTTP/1.1、RFC 6455 WebSocket、DNS 与可选 `Tina::NetworkTls` 均已落地。除 DNS 外零 worker 线程（`getaddrinfo` 无可移植非阻塞版本，故用 `scheduleIo` + owner 轮询而非首版推荐的 `postMain`，后者至今零生产调用点）。2026-08-29 转 Accepted 并附「与首版的偏差」九条。**待**：Linux 十个组件的 POSIX 分支一次未编译 |

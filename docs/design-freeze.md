...
| Profiling | Tina Trace/Metrics；Tracy 用于定位，bench 用于回归 | [0002](adr/0002-tracy-and-benchmark.md) | Partial：None frontend、可选 Tracy zone adapter contract、Runtime FrameUpdate consumer 与 `tina_bench` schema v1 已落地；TRACE-002 验证收口中，Metrics 与 session/capture 控制面仍后置 |
...
| 领域 | 后置范围 | 重新开启条件 |
| --- | --- | --- |
| Render | 可配置 shadow atlas、自研 RHI | 当前 Cook-Torrance GGX/IBL + 固定4级联 directional CSM + 固定单 SpotLight shadow + 固定单 PointLight 六面全向 shadow 产品路径稳定且有 profile/视觉需求 |

...
| AssetHandle 终态 | 2D World Sprite、standalone Particle/Trail 与 TileMap emit 已存 weak Handle，并在调用期借用共享 resolver；Asset 已提供固定容量 Sprite registry，RenderDevice 实例分配唯一/单调 key，产品手写与2D持久 binding key 均已删除；3D 与 FrameResourceRef 未迁移 | A1/A2/A3/A4 已完成；总项仍 Partial，3D Handle 与 FrameResourceRef 终态后置 |

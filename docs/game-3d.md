...
仍是低层 direct/fallback SPI。point/spot shadow 与可配置 shadow atlas 仍属 `RENDER-001` 后续。
...
`--ui-theme-demo` 在产品门禁中执行 Dark→Light→Dark 和2次 collection step；当前退出 schema 12 验证
...
| `tina_sample_3d` | 双 mesh glTF→MikkTSpace tangent→Cooked P3N3T4UV2→AssetSystem→weak Handle Prefab/Scene→Mesh3D registry→packet-local geometry/material ref→bgfx tangent TBN；Mesh/Material/共享 Texture 统一 owner、原子 material bundle + Cook-Torrance GGX、cooked EnvironmentMap split-sum IBL、3个 World DirectionalLight3D（1个固定4级联 CSM），PointLight3D 与 SpotLight3D 各2个可见+1个裁剪的逐帧 snapshot、deterministic pass scheduler、成熟 retained controls、虚拟化产品数据与事务换肤 | point/spot shadow、可配置 shadow atlas |
...
`sceneExtract`、`evidenceSchema=12`、`tangentMeshesUploaded=2`、`cookedEnvironmentMap=true`、
...
`submittedCascadedDirectionalShadowCascadeCount=4`、mesh/material handle 发布数、`meshBindingsRegistered=2`、
...
- Cooked Material v2 写入 metallic/roughness factor 与可选 MR/normal Texture2D deps；Runtime/bgfx
  ...
  directional CSM，尚无 point/spot shadow、transparent 或 post；
...
下一步只在可执行 Backlog 中维护：`RENDER-001` 剩余（point/spot shadow 与可配置 shadow atlas），

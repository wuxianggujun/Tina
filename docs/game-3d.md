...
tina_sample_3d [--frames=N] [--frame-delay-ms=N] [--gltf=<path>|--gltf <path>]
               ...
               [--ibl=on|off] [--point-shadow=on|off] [--help]
```
...
`--ui-theme-demo` 在产品门禁中执行 Dark→Light→Dark 和2次 collection step；当前退出 schema 14 验证
...
`sceneExtract`、`evidenceSchema=14`、`tangentMeshesUploaded=2`、`cookedEnvironmentMap=true`、
...
`submittedPointLightShadowCount=1`、mesh/material handle 发布数、`meshBindingsRegistered=2`、
...
从 `1/1` 变为 `0/0`，中央 3D RGB ROI fingerprint 改变且逐像素 L1 大于0。临时 raw capture 在比较后删除。

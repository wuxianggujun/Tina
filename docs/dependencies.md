...
| Box2D 3.x | 唯一 2D Physics backend | vcpkg `physics2d` feature；Legacy 另由 `legacy` feature 持有 | `tina_physics2d` PRIVATE | M11-A0–A3 直接门禁已过（lifecycle + contact + query + deferred command）；产品接入后置 |

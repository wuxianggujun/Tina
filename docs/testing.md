...
| Windows 11 / MSVC 19.50 / CMake 4.2.3 | M11-B1 TileChunk dirty cache | Debug C++23 | 6/6 asset | `TileChunkDirtyCacheTests`：首帧 rebuild→次帧 hit；`setTile` 只脏一 chunk；同值 set 无 revision；300 帧 pan+edit `rebuilds<<visible`；clear 全量重建；capacity=0 拒绝 |

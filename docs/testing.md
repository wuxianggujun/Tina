...
| Windows 11 / MSVC 19.50 / CMake 4.2.3 | M11-E5 textured Unlit | Debug C++23 | Material*Texture* + CatalogCookTests.MaterialUnlitWithTexture* + sample | dep flag；`texturedUnlit`；`texturesUploaded=1` |
...
| `tina_sample_3d_infrastructure` | M9-B 最小 bgfx Opaque3D fixture；M8-D1 可经 Scene extract 接线 | Desktop bootstrap + bgfx；全 surface clear View 0、depth-tested procedural Cube View 1；默认/门禁300帧，当前每帧3个 Cube 和1个 instance batch；JSON 可含 `sceneExtract` | 只接受 fixture key `mesh=1/material=1/submesh=0`；canonical `P3_N3_UV2` 静态 VB/IB + unlit shader + transient instance buffer；不证明 Cooked Mesh/Material/Texture/Prefab、通用 Pipeline/PBR、Pass Scheduler 或正式3D产品 |
| `tina_sample_3d` | M11-E9 glTF/Prefab 产品 smoke | 临时 glTF → cgltf cook → Catalog Mesh/Material/Prefab → GPU mesh bind → instantiatePrefab → Scene extract + UI；JSON `gltfCooked`/`prefabInstantiated`/`sceneExtract` | 首 primitive / solid Unlit / fixture meshKey；**不单独**满足 M12（仍需 2D/UI/Audio/Linux/零引用） |

...
| Windows 11 / MSVC 19.50 / CMake 4.2.3 | M10-A2b Catalog file load | Debug/Release C++23 | 见实现门禁 | Core `readFile` 与 Asset Manifest 文件→Snapshot；覆盖空路径/不存在/超限/合法加载/失败回滚。不实现 Handle/Lease、async IO 或 Cooker |

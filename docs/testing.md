...
完整 D2 Windows Debug/Release 为53/53；listener extension 的最新 Windows Debug 为55/55。
Windows Release 与 Linux 未在本轮重跑；Linux 仍保留 b3e 46/46，Clang 无诊断。这个 executable 必须与 `tina_ui_tests`、基础 `tina_tests`
分开报告；它不通过 CTest 调度。

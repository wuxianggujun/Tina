...
- `TINA_BUILD_TESTING=ON` 时 CMake 始终生成 vNext 基础 `tina_tests`；Legacy ON 时另行生成
  ...
  混入同一最终二进制；M7-C1b/C1c-a/C1c-b1/C1c-b2 与 b3d2 低层 updater UI 树、布局、命中快照、
  point query 与 synthetic route 核心另有独立 `tina_ui_tests`，M7-C1c-b3b/b3c/b3d1/b3d2
  Runtime→vNext UI producer、primary-window owner、layout coordinator 与 scoped Game SDK access 另有独立
...
| --- | --- | --- | --- | --- |
| Windows 11 / MSVC 19.50 / CMake 4.2.3 | vNext M6-A/M7-A/M7-B1/M7-B2/M7-C1b/C1c-a/C1c-b1/C1c-b2/C1c-b3a/C1c-b3b/C1c-b3c/C1c-b3d1/C1c-b3d2：Core/Platform/Input/Task/Render/Runtime/UI、startup UI seed/scoped Game SDK capability、WindowSurface handoff、Desktop bootstrap、真实 bgfx backend | Debug C++23 | 194/194 | Null 图另通过独立 UI 78/78、Runtime→UI 42/42与Null样例300帧；独立 adapter 通过GLFW 25/25、bgfx 11/11、GLFW/Desktop样例各300帧 |
| Windows 11 / MSVC 19.50 / CMake 4.2.3 | vNext M6-A/M7-A/M7-B1/M7-B2/M7-C1b/C1c-a/C1c-b1/C1c-b2/C1c-b3a/C1c-b3b/C1c-b3c/C1c-b3d1/C1c-b3d2：Core/Platform/Input/Task/Render/Runtime/UI、startup UI seed/scoped Game SDK capability、WindowSurface handoff、Desktop bootstrap、真实 bgfx backend | Release C++23 | 194/194 | Null 图另通过独立 UI 78/78、Runtime→UI 42/42与Null样例300帧；独立 adapter 通过GLFW 25/25、bgfx 11/11、GLFW/Desktop样例各300帧，Desktop输出 clean status ok |
...
| Ubuntu 22.04 / GCC 13.4 | vNext 至 C1c-b3d2：event-time Pointer/UI tree/layout/committed-hit/query/route core、Runtime startup seed/scoped Game SDK UI capability，Legacy/真实 bgfx backend 关闭 | Debug C++23 | 194/194 | 当前 Null 图UI 78/78、Runtime→UI 42/42、Null样例300帧；X11 GLFW 23/23为前序门禁 |
| Ubuntu 22.04 / Clang 22.1.8 + libstdc++15.2 | vNext 至 C1c-b3d2：event-time Pointer/UI tree/layout/committed-hit/query/route core、Runtime startup seed/scoped Game SDK UI capability，ASan/UBSan/LSan | Debug C++23 | 194/194 | 当前 Null sanitizer 图UI 78/78、Runtime→UI 42/42、Null样例300帧且无诊断；X11 GLFW 23/23与精确第三方 XIM suppression为前序门禁 |
...
| --- | ---: | ---: | ---: | --- |
| Windows 11 / MSVC 19.50 / CMake 4.2.3 Debug | 194/194 | 25/25 | 11/11 | C1c-b3d2 Null 图另通过独立 UI 78/78、Runtime→UI 42/42与Null样例300帧；GLFW/Desktop样例各300帧，前序 WindowSurface GLFW样例1800帧仍保留 |
| Windows 11 / MSVC 19.50 / CMake 4.2.3 Release | 194/194 | 25/25 | 11/11 | C1c-b3d2 Null 图另通过独立 UI 78/78、Runtime→UI 42/42与Null样例300帧；GLFW/Desktop样例各300帧，Desktop输出 clean status ok |
| Windows 11 / MSVC 19.50 / CMake 4.2.3 production-style | 测试 target 关闭 | 不构建 | 不构建 | 更早门禁：`TINA_BUILD_TESTING=OFF`，GLFW样例300帧返回0 |
| Ubuntu 22.04 / GCC 13.4 vNext Null；前序 GLFW X11 | 194/194 | 23/23（C1c-b3a） | 未运行 | C1c-b3d2 Null 图直接通过独立 UI 78/78、独立 Runtime→UI 42/42与Null样例300帧；adapter样例保留历史门禁 |
| Ubuntu 22.04 / Clang 22.1.8 + libstdc++15.2 vNext Null + ASan/UBSan/LSan；前序 GLFW X11 | 194/194 | 23/23（C1c-b3a） | 未运行 | C1c-b3d2 Null sanitizer 图直接通过独立 UI 78/78、独立 Runtime→UI 42/42与Null样例300帧且无诊断；C1c-b3a GLFW仅精确抑制第三方 `_XimOpenIM`，13次/5304 B |
...
当前 b3d2 Null 门禁在 Windows MSVC Debug/Release、Linux GCC 13.4 与 Linux Clang 22 sanitizer 都是
194项基础测试；独立 UI 为78项，Runtime→UI为42项，Null样例为300帧。前序 C1c-b3a 的23项 GLFW
专项覆盖 Button/Wheel 事件时 logical position、非有限坐标拒绝、Move 语义边界和
GLFW `A → Button/Wheel → B` 的帧末位置隔离。
M7-C1c-b3d2 新增 startup seed 与 scoped Game SDK UI capability 后，Windows Debug/Release 均通过
基础 `tina_tests` 194、独立 `tina_ui_tests` 78、独立 `tina_runtime_ui_tests` 42；GLFW专项25、bgfx专项11。
Linux GCC 13.4 与 Clang sanitizer 均通过相同194/78/42与Null样例300帧，Clang无诊断。
...
b3d2 root-scoped child 创建使用独立 `tina_ui_tests`。当前 Windows 11 / MSVC 19.50 Debug/Release、Linux
GCC 13.4 与 Clang 22.1.8 + libstdc++15.2 ASan/UBSan/LSan 均直接 GoogleTest 78/78 通过；Clang 无
...
hole、63/64 bit 边界、Button/Wheel 事件时位置、reset/cancel/非 Pointer 不路由、不伪造 Up、claims 恒
...
当前 C1c-b3d2 Headless 构建的 `tina_sample_null` 已在 Windows Debug/Release、Linux GCC 13.4
...
- UI 后续：在 b3d2 startup/root capability 基础上补 Widget owner、Label 文本、Button default action、
...
- M7-C1c-b3b/b3c/b3d1/b3d2 已把 Move/Button/Wheel routed consumption、primary-window `UIContext`
  ownership、bounded capacity、每帧 layout commit、startup seed 与 Game SDK root owner 放进独立/生命周期门禁；
  后续仍需真实 continuous claim 与 capture/focus/modal 取消；M7-E GLFW Gamepad 只验证相邻 Poll sampled diff，
  实体矩阵和回滞/重复；
...
当前记录为 Windows MSVC 19.50 Debug/Release、Linux GCC 13.4 与 Clang 22 sanitizer 均78/78。

Windows M7-C1c-b3b/b3c/b3d1/b3d2 Runtime→vNext UI producer、primary-window owner、layout coordinator 与
scoped Game SDK UI access 的独立直接门禁为：
...
Linux GCC 与 Clang sanitizer 均为29/29。当前 b3d2 在 Windows Debug/Release、Linux GCC 与 Clang
sanitizer 均为42/42。这个 executable 必须与 `tina_ui_tests`、基础 `tina_tests`
分开报告；它不通过 CTest 调度。
...
当前 Windows C1c-b3d2 Null 结果是 Debug/Release 均基础194/194、UI 78/78、Runtime→UI 42/42与
...
Linux 当前 C1c-b3d2 Null 为 GCC 与 Clang sanitizer 基础194/194、UI78/78、Runtime→UI42/42与
...
GCC 13.4/CMake 4.2.3 已通过基础194/194、UI 78/78、Runtime→UI 42/42与Null样例300帧；
Clang 22.1.8 + libstdc++15.2 ASan/UBSan/LSan 已通过相同的194/194、78/78、42/42与
Null样例300帧，且无 sanitizer 诊断。`TINA_BUILD_SHADERS=OFF` 输出
...
| 样例 | 状态 | 主要证明 | 资源策略 |
| --- | --- | --- | --- |
| `tina_sample_null` | M6-A/M7-A/M7-B1/M7-C1c-b3d2 Headless 已实现 | EngineHost、PlatformFrame/Input/Action、单个 `IGameState`、私有 primary-window UI owner/route/layout seam、Headless/Disabled/Null、300帧生命周期；无窗口 startup seed 显式选择 Headless，UI capability 请求返回结构化 unavailable，layout coordinator 成功 no-op，Linux 10,000帧仍是上一批历史结果 | 无真实第三方 backend，也不证明 UI root/Widget/DisplayList/可见 UI |
...
| `tina_sample_desktop` | M7-B2 Desktop bootstrap + C1c-b3d2 Runtime UI owner/route/layout seam + 真实 backend smoke 已实现 | `Tina::Desktop::CreateEngine` 私有组合 SteadyClock、GLFW WindowSurface、DisabledTaskSystem 与 bgfx；默认300帧 deep-blue clear/present；startup seed 显式绑定 primary-window Context，空 UIContext 产生 `None` consumption/claims，并在 Render 前提交空 layout snapshot | 当前 b3d2 Windows D3D11 Intel Iris Xe Debug/Release 300帧均通过，Release clean；前序 Linux GCC 13.4 与 Clang 22 sanitizer Desktop 门禁也已通过；Clang WSL2 为 Vulkan/llvmpipe，不代表硬件 GPU 性能，也不代表可见 UI、Scene/Pass Scheduler 完成 |
...
当前 C1c-b3d2 Windows Desktop bgfx 可见门禁已验证 Debug/Release `tina_sample_desktop` 通过

...
当前迁移结果对应 2026-07-20 的 `codex/tina-vnext-runtime`：
...
| Windows 11 / MSVC 19.50 / CMake 4.2.3 | M10-A2a Core ContentHash digest | Debug/Release C++23 | 见实现门禁 | 基础 `tina_tests` 覆盖 XXH3-128 v1 确定性、little-endian 布局、空输入、Cooked payload verify 成功/篡改失败与公共头无 `XXH*` token。A2a 不实现文件 IO、Handle/Lease、Catalog 磁盘加载或 Cooker |

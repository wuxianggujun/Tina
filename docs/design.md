...
RGBA ImageQuad/backend、产品/失效/尺寸矩阵与 `ui_image_nineslice_v1` 性能证据已经落地；下一阶段继续扩展
唯一 UI ABI 的 Component/Behavior、StyleClass/pseudo-state 与 paint-only Motion，具体边界见
...
| 性能 | `tina_bench` schema v1 + provisional 结论；UI 50,000 节点深树 structure/layout/hit/paint 非递归回归，Popup publication 为线性步骤；UI clean/dirty/route/virtual collection 与 Image/NineSlice workload | 完整 dirty-range pruning、固定门禁机 hard gate、多进程 MAD、Component/Style/Motion workload |

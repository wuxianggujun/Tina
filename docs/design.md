...
RGBA ImageQuad/backend 链路已经落地；下一阶段先关闭图片产品/性能证据，并继续扩展唯一 UI ABI 的
Component/Behavior、StyleClass/pseudo-state 与 paint-only Motion，具体边界见 [UI 框架设计](ui-framework.md)。
...
| UI | Tree/layout/hit/route/paint/semantics、文本/Glyph、Focus Scope/Modal/Pointer Capture、ScrollView、Dropdown/Popup、虚拟 ListView/TreeView；Element/recipe authoring、固定预算多节点 build transaction、统一 RoundedRect；Image/Icon content、Canvas Image/NineSlice、root-scoped resolver/pin 与 RGBA ImageQuad；Slider Focusable/Focus semantics、RangeInput Arrow/D-pad 独立调值 command 与 Dark/Light 交互状态矩阵已关闭；accessibility action seam、Windows UIA Invoke/Toggle/RangeValue/Value patterns、HWND 桥接与跨进程 action gate | Runtime component transaction、图片产品采用与 `ui_image_nineslice_v1` 性能/视觉证据、StyleClass/pseudo-state stylesheet、paint-only Motion；多行/grapheme/BiDi/复杂 shaping、完整 IME 候选窗、Narrator/Inspect 金标、AT-SPI |
...
| 性能 | `tina_bench` schema v1 + provisional 结论；UI 50,000 节点深树 structure/layout/hit/paint 非递归回归，Popup publication 为线性步骤 | 完整 dirty-range pruning、固定门禁机 hard gate、多进程 MAD、更多 workload |

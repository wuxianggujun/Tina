...
一个实施后才明确的结论：**UDP 切片不触发 D3 与 D7**。非阻塞 socket 加每帧 `receive()`
轮询不需要 worker 线程，因此没有 worker→owner marshal 问题，也没有在途操作可供取消。
这两项留给未来的 TCP/HTTP 切片，届时 `postMain` 仍将是首个生产用户（至今 `src/`、
`samples/`、`include/` 零调用点）。
...
| D3 | 完成通知机制 | **owner 持有状态 + `pump()` 轮询**（Asset 式，但乱序） | `postMain`/`pumpMain`：成为 C3 的首个用户，需先证明其容量/失败语义；回调直接在 worker 线程触发：违反 owner-thread 契约，被排除 |
...
| D7 | 取消语义 | **句柄级 cancel，语义为"不再投递完成"** | 真正中断在途 syscall 不可移植；承诺"已取消即未发送"做不到。必须明确 cancel 后副作用可能已发生 |
| D8 | 完成顺序 | **允许乱序，按完成时序投递** | 严格 FIFO：见背景，队头阻塞 |
| D9 | 可靠 UDP / netcode 是否在范围内 | **不在** | 序号/ack/重传/分片重组/拥塞控制/快照 delta/客户端预测是独立子系统，需单独 ADR |
| D10 | 第三方选型 | **待 D1 确定后再定** | UDP 切片可零第三方（平台 socket API）；HTTP 需 curl 或平台原生；注意 libuv 已因 lws 被链入但项目从不调用，不构成可用基础 |
...
`Create` 时捕获。worker 线程只写入自己拥有的 per-request 状态，不触碰任何共享
容器。

...
| NET-001 | InProgress：`Tina::Network` 首切片已落地严格数值 IP 地址与 owner-thread 固定容量非阻塞 UDP datagram socket，零第三方依赖，公开头不含 socket 类型。因非阻塞 + 每帧 `receive()` 轮询，本切片不需要 worker 线程，故 ADR 0033 的 D3（worker→owner marshal，`postMain` 至今零生产调用点）与 D7（取消）留给未来 TCP/HTTP 切片。`tina_network_tests` 47/47，同轮 `tina_tests` 413/413、`tina_ui_tests` 836/836、`tina_runtime_ui_tests` 151/151、`tina_sample_null` 300 帧 exit 0，SDK 头扫描 292/292。Linux（Docker GCC13）POSIX 分支已验证：编译零警告、当轮 35/35 与 Windows 一致，边界值与超限用例均通过（自审后续新增的 12 个用例尚未在 Linux 复跑）。三轮自审共修六个缺陷，含 `MaximumDatagramBytes` 静默丢包与 `WinsockScope` refcount 未真正同步。**待：** ADR 0033 追认；真实跨主机网络行为与 `WouldBlock` 路径在 loopback 上无法覆盖 |
...
| ASSET-ID-001 | Planned：用 versioned canonical root-relative 派生替代 glTF exact-path legacy XOR，明确 importer-version full recook/migration，并补 project move 与构造碰撞门禁；media 单 Texture2D 输出的双 FNV-1a 现状不等于该任务已关闭 |

...
| ASSET-HANDLE-SCENE-2D-A2 / N11 | fixed-capacity owner-thread Sprite registry 借用 Store/device；device-instance allocator 事务分配唯一、单调不复用 key，同 device 多 registry 安全；唯一 required Texture2D cooked dependency fail-closed resolve；产品 State unbind 后 destroy；Theme 接入时 schema 13 保留 binding/release/texture destroy/resolver evidence，Scene Explorer 接入后的当前 schema 14 继续覆盖这些字段 |
...
| TEST-003 | bgfx + FreeType 图同轮运行 Core/Scene/Asset/Render/UI 模块测试和 300 帧 product-3d；schema 4 同时验证 glTF/PBR/3-light/registry、Asset ListView/Scene TreeView 与 Dark→Light→Dark retained UI |
...
| 3D product | 双 mesh + Resources-owned AssetStore + Prefab/Scene weak mesh/material Handle + engine-provided、State-owned Mesh3D registry + extract-time resolver、原子 material bundle、baseColor/MR/normal 贴图采样、material factors、有界0..4 directional lights 已有证据（产品提交3灯）；schema 4 进一步证明成熟 retained controls、Asset ListView/Scene TreeView、Dark→Light→Dark、注册/释放/销毁与 registry 释放 | 统一 `FrameResourceRef`/retirement owner；RENDER-001 的完整 PBR/IBL/shadow/light component/pass scheduling |

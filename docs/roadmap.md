...
| RENDER-001-VERTEX-TANGENTS | StaticMesh v1 以 `vertexLayout` 兼容 P3N3UV2/P3N3T4UV2；glTF authored/MikkTSpace tangent、Asset upload、Null/bgfx layout、signed-scale-correct shader TBN 与旧布局 derivative fallback 贯通；product-3d schema 9 证明2份 tangent mesh upload |
...
| TEST-003 | bgfx + FreeType 图同轮运行 Core/Scene/Asset/Render/UI 模块测试和 300 帧 product-3d；当前 schema 9 同时验证 glTF/PBR、2份 P3N3T4UV2 tangent mesh upload、实时 surface/camera aspect、responsive UI、3个 directional light、PointLight3D/SpotLight3D 各自 authored/committed/culled=`3/2/1` 与稳定提交、Mesh/Material/共享 Texture owner retirement、packet-local resolver、Asset ListView/Scene TreeView 与 Dark→Light→Dark retained UI |
...
| 3D product | 双 mesh + authored/MikkTSpace tangent + P3N3UV2/P3N3T4UV2 + Resources-owned AssetSystem + Prefab/Scene weak mesh/material Handle + engine-provided、State-owned Mesh3D registry + packet-local geometry/material resolver、Mesh/Material/共享 Texture 统一 owner、原子 material bundle、baseColor/MR/normal 贴图采样、material factors、World DirectionalLight3D/PointLight3D/SpotLight3D→逐帧 RenderScene snapshot 与 point/spot influence-sphere culling 已有证据；schema 9 继承实时 surface/camera aspect、responsive right rail/footer、3个 directional light，以及 PointLight3D/SpotLight3D 各自 authored/committed/culled=`3/2/1` 连续300帧稳定提交、2份 tangent mesh upload、2 Mesh/2 Material/3 Texture handoff、weak handle 失效、ledger Released、Asset ListView/Scene TreeView 与 Dark→Light→Dark | RENDER-001 的完整 PBR/IBL/shadow、pass scheduling |

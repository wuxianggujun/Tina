...
| 2D-EDITOR | World2D authoring document、固定预算 revision history、editor shell 的完整布局骨架、edit/undo/redo 与 Scene runtime snapshot preview 已落地；下一步是原子文件保存、真实 viewport、TileMap 和动画工具 |
...
- TileMap/动画专用 authoring document、Inspector 字段 transaction、文件原子保存与真实产品 cook/viewport；World2D document/undo 和 editor shell 布局骨架已进入 Next；

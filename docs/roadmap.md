...
| Backlog | 目标 | 为什么现在做 |
| --- | --- | --- |
| ASSET-HANDLE-SCENE / N16.4 | 将 Mesh3D/Material 迁移到 packet-local resource，并统一 Mesh/共享 Texture retirement owner | N16.3 已关闭 Sprite2D Lease/GPU/binding 分裂 owner；3D 仍保留 registry key、registered flag 与调用方 GPU cleanup 账簿 |

...
| 2D-EDITOR | EditorApp 独立模块、World2D + Prefab v2 World3D canonical document、固定预算 revision history、独立 workspace path/loaded/baseline/dirty session、2D/3D 完整 TRS Inspector transaction、routed-pointer viewport gizmo、edit/undo/redo、原子 Save、Scene runtime preview 与 Sprite/Mesh GPU viewport 已落地；下一步是 Catalog asset-resolved viewport、TileMap 和动画工具 |
...
- TileMap/动画专用 authoring document、Catalog asset-resolved viewport 与真实产品 cook；World2D/World3D document、独立 workspace session、完整 TRS、viewport gizmo、原子 Save 和 EditorApp 布局已进入 Next；

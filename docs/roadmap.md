...
| Backlog | 目标 |
| --- | --- |
| GAMEPLAY2D-001 | InProgress：`Scene2DRuntime` 实例化四种 authored resource 节点、固定每帧顺序，并经 `fixedUpdatePhysics()` 统一驱动 `Scene2DPhysicsBridge`（`step → applyTo → updateWorldTransforms`）；`tileMap()`/`tileLayers()` 暴露 resident 地图给 GridCollision/PhysicsSync/DirtyCache/picking，runtime 因此不可移动。ADR 0031 的 D1-D7 已全部落地。`tina_physics2d_tests` 76/76（含 Scene2DRuntime 15）。**待：** ADR 0030/0031 仍是 Proposed；没有任何产品/sample 链接 `Tina::Gameplay2D`，因此 product-2d gate 尚未覆盖该 owner |

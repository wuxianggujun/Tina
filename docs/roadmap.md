...
| Backlog | 目标 |
| --- | --- |
| GAMEPLAY2D-001 | InProgress：`Scene2DPhysicsBridge` 把 authored physics 桥到 `PhysicsWorld2D`（单向 authoritative），`Scene2DRuntime` 实例化四种 resource 节点并固定每帧顺序。`tina_physics2d_tests` 72/72（含 Scene2DRuntime 11）。**待：** ADR 0030/0031 仍是 Proposed；ADR 0031 的 D5（由 `Scene2DRuntime` 统一驱动物理桥）未实现；没有任何产品/sample 链接 `Tina::Gameplay2D`，因此 product-2d gate 尚未覆盖该 owner |

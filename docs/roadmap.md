...
| 3D-SKIN-001 | 3D 骨骼动画 lane：glTF skin/animation cook → SkinnedMesh/AnimationClip3D asset → Animator3D → GPU skinning → product-3d 证据（A1–A4 切片验收见 Backlog） |
| RENDER-002-TRANSPARENT | 3D transparent pass：cook 期声明、back-to-front 稳定排序、固定 pass 顺序与 product-3d 交叠差分证据 |
...
- 3D post-process（HDR/tone mapping）与跨 GPU golden（transparent pass 已提升为 `RENDER-002-TRANSPARENT`）；

...
| 2D-EDITOR | EditorApp 独立模块、World2D + Prefab v2 World3D + TileMap v3/v1 canonical document、固定预算 revision history、独立 workspace session、完整 TRS Inspector transaction、viewport gizmo、Tile Paint/Erase、layer/object authoring、root+chunk cook preview、原子 Save，以及 `AssetSystem` + Sprite/Tileset/Mesh registry 驱动的 Catalog-resolved 2D/3D GPU viewport 已落地；下一步是 SpriteAnimationClip timeline 工具 |
...
- SpriteAnimationClip timeline authoring/cook；World2D/World3D/TileMap document、独立 workspace session、完整 TRS、viewport gizmo、Tile brush/layer/cook preview、原子 Save、Catalog-resolved viewport 和 EditorApp 布局已进入 Next；

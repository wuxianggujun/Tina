...
| EDITOR-E1-TILE-PALETTE | InProgress：Tileset cooked metadata 驱动固定容量 Tile Palette；统一 Editor tests/smoke 已完成，待手动多次选择/绘制验收 |
| EDITOR-E2-SETTINGS | InProgress：Editor 私有 versioned UTF-8 settings carrier 已接入布局/snap/recent；统一 Editor tests/smoke 已完成，Preferences UI 后置，待跨重启验收 |
| EDITOR-E3-RECENT | InProgress：Start Center 与 File/Open Recent 已接入 Catalog switch 提交点；统一 Editor tests/smoke 已完成，待手动项目切换与失效移除验收 |
| 2D-EDITOR | 2D/3D Editor 产品闭环：四类 current-schema authoring document/file、bounded history、per-tab session、Save/Save As/dirty-close、Project Browser/New/Open + live Catalog switch、事务化 source import 与一步 PNG/JPEG/WAV 导入、Godot 式 Node registry/Hierarchy 拖放、`EditorNodePropertyOperations` 节点专属属性（含 Physics Body/Collision Shape Inspector）、TRS Inspector、viewport navigation/grid/gizmo/marquee、PlaySession、frame shortcuts 与 Line/Ellipse 视口视觉均已落地；旧 Components/Add/Remove 与空节点包装 API 已破坏式删除，实现明细与本轮证据见 [Backlog](backlog.md)。**2026-08-24 Windows 统一证据：** `tina_editor_tests` 112/112、`tina_editor_app_tests` 23/23、`tina_tests` 377/377；`TinaEditor.exe --auto-demo --frames=70` 的 2D/3D smoke 均 exit 0。**待：** 跨 DPI/GPU 视觉金标与 Linux `zenity`/`kdialog` 真实 dialog 产品门禁；完成前保持 InProgress |
...
- 玩法脚本（`SCRIPT-001`，[ADR 0045](adr/0045-script-module-boundaries.md) Proposed）。只做 Luau；脚本是 `IGameState` 的客人，不是第二套引擎。`require` 走 cooked 依赖图（与 Fx2D 同类），宿主 API 白名单默认拒绝。确认前不占位、不引入 VM。进入 Next 的条件是真实产品需要「不重编 C++ 也能改控制流」，并带上容量、失败语义和验收命令；
- 移动端（Android/iOS）平台后端（`MOBILE-001`，[ADR 0032](adr/0032-mobile-platform-contract-boundaries.md) Proposed）。后端本身只有 7 个纯虚；成本在六个桌面契约上（单 pointer、指针位置永久存在、native surface 不变、poll 线程==渲染线程、只有三个 renderer、preedit 由应用控制）。**切片 A 已完成**（2026-08-28 更正，此前写作最大单项前置工作）：pointer presence（`3569f0b4`）、多点触控路由（`4ec987a9`）与 per-pointer cancel 作用域均已在桌面落地并有单测，`UIVirtualStick` 是首个真实多点消费者，故前两个契约已不再是阻塞项。per-pointer cancel 补的是 presence 与多点之间的接缝：平台此前能报「某根手指抬起」，而 UI 只能全量取消八个 slot —— 在触摸设备上一根手指抬起会让另一根丢掉控件。剩余四个契约与后端实现仍待 ADR 0032 的 D3（`run()` 保持阻塞还是改外部驱动 `tick()`）定论，未定则 iOS 无法开工；切片 B 的三项（surface 重建事件、软键盘 show/hide 与遮挡矩形、Metal/ESSL shader profile）经核实**均为零实现**。**更正（2026-09-01）：** 上一句整句**已过期**（原文保留）。①**D3 已于 2026-08-28 定案**：maintainer 选择外部驱动，`EngineHost::start()`/`tick()` 已落地（`include/tina/runtime/EngineHost.hpp:50-51`），`run()` 变 `while` 封装故 75 处调用点零改动，ADR 0032 的 D3 行已记为「已定」；iOS 的形态阻塞因此解除。②**切片 B 三项已全部完成**：C3 surface 重建走 `nativeBindingRevision`（另立 [ADR 0034](adr/0034-native-surface-rebind.md)）、C5 ESSL/OpenGLES profile 与查表、C6 软键盘 show/hide + 宿主上报遮挡高度 + preedit 四阶段映射（`AndroidCompositionSession`）。③**Android 后端已在模拟器实机运行**（非仅编译）：`src/platform/android/` 实现全部 7 个纯虚，JNI 桥 + AGP 工程产出可安装 APK，`tina_platform_android_tests` **80/80 在 `emulator-5554`（Android 36 x86_64）实机通过**，触摸/按键/文本/preedit 四条输入链与旋转、后台前台循环下的 surface rebind 均端到端跑通。逐轮证据见 [Backlog](backlog.md) `MOBILE-001`。**2026-09-06 手柄源码更新：** Android `InputDevice` / iOS `GCController` 已接入共用的有界队列与 generation 状态机，包含按钮/摇杆/扳机、hat 去重、热插拔、挂起和溢出重同步；原生输入进入既有 UI/ActionMapper。新增测试源码已接线，尚无本批执行或真机证据，`MOBILE-001` 仍为 InProgress。**更正（2026-09-05）：** 本行此前写作「**iOS 一行未写**（仓库内无任何 iOS 平台代码，`.mm`/`CADisplayLink`/`CAMetalLayer` 仅出现在 bgfx 第三方示例里），故「移动端」目前实际只等于 Android」，**整句已过期**：`src/platform/ios/` 有 `IosPlatform`/`IosSession`/`IosInputBridge`/`IosCompositionSession` 实现并经 `src/platform/CMakeLists.txt:40` 注册；`ios/app/` 四个 `.mm` 里 `TinaViewController.mm:82` 建 `CADisplayLink`、`TinaMetalView.h:9` 是 `CAMetalLayer` 视图；`tests/platform_ios/` 四个测试文件在每个宿主上都编译。iOS 尚缺的是**真机验收**，不是代码。同期 `design-freeze.md` 的「Metal 仍未做」亦已过期：`cmake/TinaBgfxEmbeddedShaders.cmake:59` 追加 `mtl|ios|metal` profile，注释记 2026-09-03 已验证 11 个 shader 全部 cook 通过；真实设备上的 Vulkan 路径未验证（模拟器 `vulkan.ranchu.so` 驱动崩溃，只跑通 GLES）；候选窗跟随需人工验收（归 `TEXT-001`）；移动端资源打包与 CI 明确不在范围。ADR 0032 整体仍为 **Proposed**（D3 单点已定案）。
...
| 2D-SERIALIZATION | World2D snapshot 唯一现行 schema v5（464-byte entity record，含 SpriteRenderer2D 自定义 shader `AssetId`，固定容量 UTF-8 节点名）（32-byte header、256-byte entity，含 SpriteAnimation 绑定；4096 entity/4 MiB gameplay blob 上限）；capture 确定排序并拒绝3D有损写入，restore 全量预检、失败 subtree rollback；旧 schema 拒绝 |
...
| UI-STATE-FEEDBACK | Slider、Checkbox/RadioButton、List/Tree 与 TextEdit 复用唯一交互/焦点状态源并完成 stale-state 清理；Windows MSVC/bgfx/FreeType Dark/Light 产品门禁22项差分检查全部通过，证据见 [UI-STATE-FEEDBACK Windows Evidence](evidence/ui-state-feedback-evidence-windows.md) |
...
| RENDER-001（全部子切片） | Opaque3D lighting/shadow/tangent/IBL 已闭环：`Mesh3DLightingDesc` 有界 0..4 directional + 0..8 point + 0..8 spot；point/spot influence sphere 在容量检查前按 active `PerspectiveCamera3D` frustum cull；固定4级联 CSM、单 SpotLight shadow、单 PointLight 六面 shadow 与 startup-only `ShadowMapExtentConfig`（`[128,4096]` 内2次幂，默认 `1024/1024/512`）；StaticMesh v1 固定 P3N3T4UV2 与 signed-scale-correct TBN；EnvironmentMap v1 + Cook-Torrance GGX split-sum IBL；product-3d resize 以实时 framebuffer extent 驱动 aspect 与 light frustum。当前字段见 product-3d schema 16；历史 schema 5/6/7/8/12/15 只在 git history |
| 2D-LIGHT N1-N5 | 0..8 committed `PointLight2D` + ambient + 0..32 `ShadowOccluder2D` segment；resolved/pixel-snapped `Camera2D` 精确 circle-vs-rectangle culling（occluder 不裁剪）；source radius 0 为硬阴影、正值为固定成本 normalized-depth 连续 penumbra；Sprite2D optional weak normal Texture2D 经独立 resolver 进入 packet-local ref，`(base, normal)` 连续 batch 与 derivative TBN 贯通。当前字段见 product-2d schema 29（继承 16..19）；**待**：跨 GPU lighting exact golden |
...
| AUDIO-RESIDENCY-001 | AudioClip v2 MemoryPcm / EncodedStream；owner-thread EncodedPcmStreamer；v1 拒绝；SDK 0.5.0 |

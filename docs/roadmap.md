...
| NET-001 | InProgress：`Tina::Network` 首切片已落地严格数值 IP 地址与 owner-thread 固定容量非阻塞 UDP datagram socket，零第三方依赖，公开头不含 socket 类型。因非阻塞 + 每帧 `receive()` 轮询，本切片不需要 worker 线程，故 ADR 0033 的 D3（worker→owner marshal，`postMain` 至今零生产调用点）与 D7（取消）留给未来 TCP/HTTP 切片。`tina_network_tests` 35/35，同轮 `tina_tests` 411/411、`tina_ui_tests` 836/836、`tina_runtime_ui_tests` 151/151、`tina_sample_null` 300 帧 exit 0，SDK 头扫描 292/292。**待：** ADR 0033 追认；Linux/POSIX 分支未在 Linux 验证；真实跨主机网络行为与 `WouldBlock` 路径在 loopback 上无法覆盖 |

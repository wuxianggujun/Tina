...
| ASSET-002 | 分阶段建立热重载与增量 Cooker；已完成 manifest revision polling、immutable Catalog change planning、fresh stage validation、idle-safe root reload、current-only source import state/planner、importer provenance capture + validated state commit，以及单 unit clean whole-package reuse | caller-driven poll 保留 baseline/candidate 重试语义；tool-side state 绑定 Catalog revision 并按 stable UnitId/read extent 检测 source/importer/output contract 变化；clean fast path 不解析 importer、不读 object、不改写 package/state；后续再接 OS watcher、多 unit dirty-unit executor 与 live Handle/Lease/GPU owner 迁移 |
...
provenance capture + validated state commit、单 unit clean whole-package reuse、fresh stage full validation 与保守的
idle-safe root reload；退出 Now 前还需要 OS watcher、多 unit clean/dirty object executor 与 live Handle/Lease/GPU owner 迁移事务，
...
| TEST-003 | bgfx + FreeType 图同轮运行 Core/Scene/Asset/Render/UI 模块测试和 300 帧 product-3d；当前 schema 13 同时验证 glTF/PBR、cooked EnvironmentMap IBL 上传/bind/clear/retire、2份 P3N3T4UV2 tangent mesh upload、固定4级联 CSM config authored/submitted=`1`、固定 SpotLight shadow authored/submitted=`1/1` 与跨帧稳定 descriptor、实时 surface/camera aspect、responsive UI、3个 directional light、PointLight3D/SpotLight3D 各自 authored/committed/culled=`3/2/1` 与稳定提交、Mesh/Material/共享 Texture owner retirement、packet-local resolver、Asset ListView/Scene TreeView 与 Dark→Light→Dark retained UI |

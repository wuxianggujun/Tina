...
| UI-FLOW-001 | InProgress；固定容量 Layer/Screen 栈与 Pause Back Action Router 已接入 2D 产品，后续推进多本地用户与输入设备提示 |
...
UI-FLOW-001 InProgress
  -> Layer/Screen stack + 2D Pause product path implemented
  -> Pause Back Action Router implemented
  -> local-user routing / input-device hints / wider product actions pending
...
分配和 checksum 等确定性不变量可以阻断，开发机墙钟只报告 `provisional`。`UI-FLOW-001` 已因 2D Pause
页面栈需求进入 InProgress；Layer/Screen 首切片不引入第二棵 UI 树。`SDK-001` 继续独立推进，每个新增公共
UI 切片同步扩展 consumer gate，不等待 UI Flow 的后续输入路由能力。

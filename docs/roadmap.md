...
- M10-A2n 已完成空请求=加载全部覆盖、CLI `--load-assets` 整包/选 id 加载冒烟；仍无 Handle/Lease；
...
- M10-A7 已完成 Desktop 默认 BoundedTaskSystem，以及 Null `UploadTicket`/`NullUploadLedger`
...
- M10-A10 已完成 `tina_sample_asset` 产品 smoke：Catalog 包 → BoundedTask IO → request/pump →
  Null upload → ReadyGpu + Lease；仍无 bgfx 真实纹理与 Cooker；
...
- M10-A19 已完成 recipe 内联 `texture2d`/`sprite` 语法（无需预编码 payload 文件）；
  仍无 bgfx 绘制；
...
- M10-A26 已完成 Tileset/TileMap cooked payload v1 编解码、cooked helpers、typed views 与
  verifyTypedPayload 扩展；运行时 TileMapInstance/chunk extraction 与 cgltf 仍后置；
- M10-A27 已完成 recipe 内联 `tileset`/`tile`/`tilemap`/`row` 语法与 cook→typed validate 闭环；
...
- M10-A30 已完成 `emitTileChunkSprites`/`emitVisibleTileMapSprites`：可见 chunk → `RenderSprite2DInput`
  列表并可 commit 进 RenderScene；独立 TileChunkRenderPacket/cgltf 仍后置；
...
- M10-A42 已完成 ActionMapper world pointer payload：只对未被 UI consume/claim 的 primary pointer
  ...
  0-step。更大范围 world-pick Game SDK API 仍后置；
- M10-A43 已完成正式 `tina_sample_2d` 样例选格消费者：Primary Pointer Primary Button 绑定
  Simulation Action，`fixedUpdate()` 只消费 `Pressed` 的 A42 锁存 `WorldPointerSample`；按
  TileMap map-local bottom-left 原点、`cellSizeMeters` 与半开地图边界转换 cell，viewport miss、地图外、
...
- M11-B2 已完成：`tina_sample_2d` Camera2D **follow + presentation 插值**——角色 `updateFrame`
  后写 previous/current；extract 用 `FrameTiming.interpolation` lerp；map 边界 clamp；
  FixedWorldHeight=4m 使 8m 地图可 pan；JSON `cameraFollowUpdates`/`min|maxCameraCenterX`/
  `lastCameraInterpolation`；RenderScene pixel snap 仍在 commit。**sample-private，不扩 Game SDK**；
...
- M11-C2 已完成：`tina_sample_2d` HUD 增加 Music/SFX 分轨音量 Slider（共 3 条）；
...
- M11-D1 已完成：后端无关 `IRenderDevice::capturePrimaryFrameRgba8` + bgfx
  `requestScreenShot`（BGRA→RGBA8 top-left）；`tina_sample_2d` 在最终 present 后捕获并输出

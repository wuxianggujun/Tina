...
| RENDER-001-VERTEX-TANGENTS | StaticMesh v1 固定为 P3N3T4UV2；glTF authored/MikkTSpace tangent、Asset upload、Null/bgfx 唯一 layout 与 signed-scale-correct shader TBN 贯通；缺 NORMAL/UV 显式失败；product-3d schema 10 证明2份 tangent mesh upload |
| RENDER-001-DIRECTIONAL-SHADOW | 最多一个 World directional shadow caster 进入 frame snapshot；独立 DirectionalShadowDepth target、1024² sampled D16、camera-slice projection、depth/normal bias 与 3×3 PCF 贯通；primary-surface clear ownership 保持独立；product-3d schema 10 证明 authored/submitted caster=`1` |
...
| TEST-003 | bgfx + FreeType 图同轮运行 Core/Scene/Asset/Render/UI 模块测试和 300 帧 product-3d；当前 schema 10 同时验证 glTF/PBR、2份 P3N3T4UV2 tangent mesh upload、directional shadow caster authored/submitted=`1`、实时 surface/camera aspect、responsive UI、3个 directional light、PointLight3D/SpotLight3D 各自 authored/committed/culled=`3/2/1` 与稳定提交、Mesh/Material/共享 Texture owner retirement、packet-local resolver、Asset ListView/Scene TreeView 与 Dark→Light→Dark retained UI |
...
| 3D product | 双 mesh + authored/MikkTSpace tangent + 唯一 P3N3T4UV2 + Resources-owned AssetSystem + Prefab/Scene weak mesh/material Handle + engine-provided、State-owned Mesh3D registry + packet-local geometry/material resolver、Mesh/Material/共享 Texture 统一 owner、原子 material bundle、baseColor/MR/normal 贴图采样、material factors、World DirectionalLight3D/PointLight3D/SpotLight3D→逐帧 RenderScene snapshot、point/spot influence-sphere culling、单 directional shadow caster 与 deterministic pass scheduler 已有证据；schema 10 继承实时 surface/camera aspect、responsive right rail/footer、3个 directional light、shadow caster authored/submitted=`1`，以及 PointLight3D/SpotLight3D 各自 authored/committed/culled=`3/2/1` 连续300帧稳定提交、2份 tangent mesh upload、2 Mesh/2 Material/3 Texture handoff、weak handle 失效、ledger Released、Asset ListView/Scene TreeView 与 Dark→Light→Dark | RENDER-001 的完整 PBR/IBL、CSM、point/spot shadow 与可配置 shadow atlas |

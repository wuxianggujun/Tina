...
| ASSET-HANDLE-SCENE-2D-A2 / N11 | fixed-capacity owner-thread Sprite registry 借用 Store/device；device-instance allocator 事务分配唯一、单调不复用 key，同 device 多 registry 安全；唯一 required Texture2D cooked dependency fail-closed resolve；产品 State unbind 后 destroy，schema 10 记录 binding/release/texture destroy/resolver evidence |
...
| 门禁 | 当前结论 | 下一关闭点 |
| --- | --- | --- |
| 2D product | Windows product-2d 同轮模块测试 + 300 帧已有证据（TEST-002）；TileMap v3 sample 每帧 demand/pump/commit visual=10 与 hidden collision=20，gameplay objects=30 留在 root；retain-window LRU、Physics sensor/joint、Advanced input、fixed-capacity Particle/Trail、World Sprite weak Handle A1 与 fixed-capacity binding registry A2 已完成；schema 10 记录2纹理注册/释放/销毁与 resolver hits | FX/TileMap component Handle 化、统一 FrameResourceRef、TileMap priority IO/editor/自动 gameplay 生成、完整 FX asset/editor/GPU simulation 与 UI-003 多 DPI 矩阵均为独立后续项 |

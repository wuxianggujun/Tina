...
| 2D-EDITOR | 2D/3D Editor 产品闭环：四类 current-schema authoring document/file、bounded history、per-tab session、Save/Save As/dirty-close、Project Browser/New/Open + live Catalog switch、事务化 source import 与一步 PNG/JPEG/WAV 导入、Godot 式 Node registry/Hierarchy 拖放、`EditorNodePropertyOperations` 节点专属属性、TRS Inspector、viewport navigation/grid/gizmo/marquee、PlaySession、frame shortcuts 与 Line/Ellipse 视口视觉均已落地；旧 Components/Add/Remove 与空节点包装 API 已破坏式删除，实现明细与本轮证据见 [Backlog](backlog.md)。**待：** 本轮统一 Editor build/gate、跨 DPI/GPU 视觉金标与 Linux `zenity`/`kdialog` 真实 dialog 产品门禁；完成前保持 InProgress |
...
| 2D-SERIALIZATION | World2D snapshot 唯一现行 schema v2（32-byte header、256-byte entity，含 SpriteAnimation 绑定；4096 entity/4 MiB gameplay blob 上限）；capture 确定排序并拒绝3D有损写入，restore 全量预检、失败 subtree rollback；旧 schema 拒绝 |

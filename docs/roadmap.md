...
| ASSET-HANDLE-SCENE-2D-A2 / N11 | fixed-capacity owner-thread Sprite registry 借用 Store/device；device-instance allocator 事务分配唯一、单调不复用 key，同 device 多 registry 安全；唯一 required Texture2D cooked dependency fail-closed resolve；产品 State unbind 后 destroy，当前 schema 13 保留 binding/release/texture destroy/resolver evidence |

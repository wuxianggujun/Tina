...
| ASSET-002 | 分阶段建立热重载与增量 Cooker；已完成 immutable Catalog change planning、fresh stage validation 与 idle-safe root reload | Cooker 可生成并完整验证独立 staging root；root API 同步复验 package、调用 change planner，并仅在 owner-thread 且 pending/in-flight、active handle、GPU upload、retirement 均为空时原子提交；后续再接 source/package detection、增量 recook 与 live Handle/Lease/GPU owner 迁移 |
...
ASSET-002 当前完成 pure planner、fresh stage full validation 与保守的 idle-safe root reload；退出 Now 前还需要
source/package detection、import metadata、增量 recook 与 live Handle/Lease/GPU owner 迁移事务，不把“空闲时可切换
root”写成“完整增量热重载已完成”。

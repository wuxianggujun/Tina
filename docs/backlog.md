...
| --- | --- | --- | --- | --- | --- | --- |
| UI-STATE-FEEDBACK | InProgress | P0 | 对齐控件交互状态、焦点契约与可辨识反馈；源码/测试切片已完成 | 现有 routed input/Focus/Theme/paint | **已完成：** Slider 的 Focusable/Focus semantics 与 runtime trait 对齐且 thumb 为 drag > focus > normal；Checkbox/RadioButton 共用单一 hover tracking；List/Tree selected overlay 复用现有 row hover/press 与 collection focus，按 pressed > hover > focus > selected 解析；TextEdit 以独立 `UITextEditPaint` 复用既有 hover/arm/focus，背景按 disabled > pressed > hover > focus > normal 解析，selection/caret 可换肤且 release/cancel 不留 stale pressed；disabled 均复用共享 opacity，既有 value/selection/IME 状态源不变。**剩余：** Dark/Light 产品视觉证据；即时反馈不得误称为动画 | Unit + Integration + Visual |
...
| UI-THEME-C | Deferred | P2 | Image/NineSlice、逐角半径、圆角 clip/backdrop 与 stylesheet 级 Theme | 不破坏已完成的统一圆角 create 默认 chrome、局部覆盖与固定容量 paint 契约 |
| 2D-LIGHT | Deferred | P2 | 2D lighting 与 shadow | **已完成 N1/N2：** backend-neutral、frame-scoped 0..8 PointLight2D + ambient + 0..32 ShadowOccluder2D segment，World 稳定提取、Null/bgfx submit preflight、bgfx 径向衰减与硬阴影、透明排序保持、product-2d 双灯双遮挡 schema 16。**待：** light culling、soft shadow/normal map 与独立视觉金标 |
...
| UI-PRODUCT-2D-THEME | product-2d Theme Button 提交 pending intent 并在 `updateUI()` 事务换肤；标准控件继承产品 chrome，Panel/标题局部层级随主题重算；当前 schema 16 仍自动 Dark→Light→Dark 与 Scene Explorer TreeView 后回到 Dark | [UI](ui.md) · [2D](game-2d.md) · [测试](testing.md) |

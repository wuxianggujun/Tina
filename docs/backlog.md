...
| TEST-003 | product-3d 同轮：Core/Scene/AssetFormat/Asset/bgfx Render/UI/RuntimeUI/bridge/FreeType 测试 + sample 300 帧；schema 4 同时验证 glTF/PBR/3-light、Mesh/Material/共享 Texture owner retirement、packet-local resolver 与自动 Dark→Light→Dark；脚本 `tools/windows/RunProduct3dGate.ps1` | [3D](game-3d.md) · [测试](testing.md) |
...
| UI-PRODUCT-3D-THEME | `Product3DUI` 独立 root；Theme Button、Auto Rotate Checkbox、Rotation Speed Slider、Frame ProgressBar 驱动实际 3D 状态；标准 chrome 继承、Panel/标题局部层级集中换肤；当前 schema 4 自动 Dark→Light→Dark，暗/亮 FreeType client capture 证据保持有效 | [UI](ui.md) · [3D](game-3d.md) · [测试](testing.md) |

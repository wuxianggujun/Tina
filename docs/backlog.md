...
| 2D-LIGHT | Deferred | P2 | 2D lighting 与 shadow | **已完成 N1：** backend-neutral、frame-scoped 0..8 PointLight2D + ambient，World 稳定提取、Null/bgfx 径向衰减、透明排序保持、product-2d 双灯 schema 15。**待：** occluder 数据、shadow 合成、light culling 与独立视觉金标 |
...
| UI-PRODUCT-2D-THEME | product-2d Theme Button 提交 pending intent 并在 `updateUI()` 事务换肤；标准控件继承产品 chrome，Panel/标题局部层级随主题重算；当前 schema 15 仍自动 Dark→Light→Dark 与 Scene Explorer TreeView 后回到 Dark | [UI](ui.md) · [2D](game-2d.md) · [测试](testing.md) |

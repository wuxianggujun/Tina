...
和 Tree/TreeItem selected semantics。当前 sample/gate 已升级到 schema 21：继承 schema 19 的 normal-map
...
Texture2D 的 owner/retirement，并以 normal on/off 各两次同机差分关闭。schema 21 另增加 Layer/Screen
注册、2D Pause Screen push/pop、Back action register/clear、单用户输入设备 revision、Pause 提示与
action/auto-resume intent 字段。旧报告不反向改写；本节不把尚未

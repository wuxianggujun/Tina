...
和 Tree/TreeItem selected semantics。当前 sample/gate 已升级到 schema 22：继承 schema 19 的 normal-map
...
注册、2D Pause Screen push/pop、Back action、单用户输入设备 revision 与 Pause 提示；schema 22 再增加
Confirm action、focused-control precedence 与 action/auto-resume intent 字段。旧报告不反向改写；本节不把尚未

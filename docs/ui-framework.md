...
| Behavior | `include/tina/ui/UIBehavior.hpp`；Activate/Toggle/RangeInput/TextInput/Scroll 使用私有 fixed-capacity side store，其余标准 capability 当前仍由 resolver 约束到私有 kind |
...
Activate/Toggle/RangeInput/TextInput/Scroll 已从 concrete kind 拆到独立 fixed-capacity side store；TextInput/Scroll resolver
仍选择现有 TextEdit/ScrollView kind 以复用输入、焦点、Pointer selection/drag 与具体视觉路径，Select 仍只接受能映射为现有
...
校验。Activate/Toggle/RangeInput/TextInput/Scroll 切片已实现五池原子预检、publish/release/reuse 与 capacity/active/high-water 统计；
Activate/Toggle/RangeInput 默认行为、TextInput selection setter/query 与 Scroll style/offset/committed metrics 按 capability。
TextInput 的 Keyboard/IME/Pointer 路由仍复用 concrete TextEdit；ScrollView paint/thumb geometry 与 Pointer drag 路由、Slider
paint/change callback/Pointer drag geometry、TextEdit paint 同样由对应 concrete kind 承担。Select 尚未迁移。官方
...
5. `UI-COMPONENT-001`：Runtime bounded component transaction 与 Activate/Toggle/RangeInput/TextInput/Scroll side store 切片已完成；
   `ui_component_build_activate_toggle_v1` 已为 Activate/Toggle 提供诚实的 256×4 前置 benchmark；后续迁移
   Select，并补统一 reservation/counter 后才发布冻结的 `ui_component_build_v1`；

...
- product-2d schema 21：继承 schema 19 的 normal-map 证据、两盏 committed `PointLight2D`、两条 `ShadowOccluder2D`，并增加 UI Flow Pause Screen、单用户设备 revision 与 Pause 提示；
...
- product-3d schema 14：2份 P3N3T4UV2 tangent mesh upload、cooked EnvironmentMap upload/IBL bind/clear/retire=`1/1/1/1`、固定4级联 CSM config authored/submitted=`1`、固定 SpotLight/PointLight shadow authored/submitted=`1/1`、PointLight shadow on/off 中央 RGB ROI 像素差分、实时 logical/framebuffer extent、相机 aspect 匹配与 responsive UI authoring，继承
...
| PBR/其他纹理通道 | Partial | metallic-roughness/normal、Cook-Torrance GGX、cooked EnvironmentMap split-sum IBL、固定4级联 CSM、固定单 SpotLight shadow、固定单 PointLight 六面全向 shadow 与三类 startup-only 可配置 D16 extent 已进 Material/Render 产品门禁；emissive/occlusion 等扩展通道后置 |

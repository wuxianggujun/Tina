...
- M7-C1b/C1c-a/C1c-b1/C1c-b2 已在独立 `tina_ui` 树核心上实现 generation `UINodeId`、`UIContext`、`UIRootOwner`
  ...
  Capture→Target→Bubble route；它尚未接入 Runtime 帧循环成为 UI producer，也尚无持久 Pointer Capture、
  Focus/Modal 或 Button default action；
...
committed hit-snapshot 数据基础、point query/反向目标选择和 synthetic Capture→Target→Bubble route。
Runtime UI producer、持久 Pointer Capture、Focus/Modal、Button default action、paint snapshot/DisplayList、
nested clip、dirty subtree pruning、FreeType 与 bgfx UI pass 仍未实现。这些能力不能从同名 Phase Context、真实 GLFW 窗口或 clear-only
...
UI Pointer/Focus 继续由每窗口 `UIContext` 路由；Gameplay Action 带目标 simulation tick，由 fixed loop
消费。

...
- Render：Null/bgfx、Sprite2D/Opaque3D Cook-Torrance GGX/IBL、UI Glyph、Texture2D/StaticMesh/EnvironmentMap
  binding、owning packet、readback retirement marker 与固定4级联 directional CSM 已有；point/spot shadow 与可配置 atlas 后置。
...
| `3D-001` / `ASSET-001` | multi-mesh E2E + URI 安全 + base/MR/normal texture sampling + Cook-Torrance GGX/cooked EnvironmentMap IBL + 固定4级联 directional CSM/pass scheduler **Done**；point/spot shadow 与可配置 atlas 后置 |

...
| `tina_ui` | Retained Tree、布局、路由输入、焦点、Widget、DisplayList、Glyph Atlas 和字体 rasterizer 接口；当前 M7-C1b 已实现树核心、route-result view ABI 与事务式 Flex-lite layout foundation | 当前只依赖 core、platform `PlatformFrameView`；后续 Font Asset 和 render 描述随 asset/render 切片接入 | bgfx/FreeType 类型、全局 UI 状态、隐式布局 |
...
Measure/Arrange、`UICommittedLayoutView` 和 structure+layout 原子发布。当前 `tina_ui` 仍只依赖
Core/Platform；changed frame 尚未按 dirty subtree 剪枝，hit route、DisplayList、widgets、FreeType、
...
4. **M7-C–E UI/IME/Gamepad**：M7-C1a 已完成 `tina_ui` 树核心、generation `UINodeId`、
   ...
   Flex-lite layout foundation。后续继续实现 dirty subtree pruning、hit route/DisplayList、
...
- M7-C1b UI 树与布局核心使用独立 `tina_ui_tests` 直接 GoogleTest；当前记录 Windows MSVC 19.50
  Debug/Release 均 39/39 通过；Linux GCC 13.4 与 Clang 22 ASan/UBSan/LSan 均基础
  `tina_tests` 183/183、`tina_ui_tests` 39/39，且无诊断；

...
`NativeWindowSurfaceLease` 和 WindowSurface-aware composition，但真实 bgfx clear/present 后端仍是
M7-B2。
...
内核、私有 GLFW 窗口子切片和 M7-B1 WindowSurface handoff 已经可独立运行；它仍没有真实 bgfx
GPU 渲染、Scene/Asset/UI/Audio 或完整状态栈，因此 Legacy 仍是当前 2D/UI/3D 产品实现，不能直接整目录删除。
...
| 完整 vNext Runtime | 尚未完成 | GameStateStack/commands、worker、Pass Scheduler/RenderFramePacket、Scene/Asset/UI/Audio、Desktop bootstrap 与 M7-B2 真实 bgfx Render backend 仍按后续切片实施 |

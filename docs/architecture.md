...
`tina_platform_glfw` adapter 和私有 `tina_render_bgfx` backend。`tina_sample_null` 不依赖真实窗口和 GPU；`tina_sample_platform`
...
内核、私有 GLFW 窗口子切片、M7-B1 WindowSurface handoff 和 M7-B2 bgfx core 已经可独立构建；它仍没有
接入产品的真实 GPU 样例、Scene/Asset/UI/Audio 或完整状态栈，因此 Legacy 仍是当前 2D/UI/3D 产品实现，不能直接整目录删除。
...
| vNext M7-B2 bgfx core | 已完成私有 clear-only backend core | `Tina::RenderBgfx` 只 PUBLIC 依赖 `Tina::Render`，bgfx 与 WindowSurface bridge 均为 PRIVATE；planner 覆盖 1×1 bootstrap、resize/resume、content-scale-only 和 suspended skip；Desktop bootstrap 与真实 GPU 门禁待接入 |
...
| 完整 vNext Runtime | 尚未完成 | GameStateStack/commands、worker、Pass Scheduler/RenderFramePacket、Scene/Asset/UI/Audio、Desktop bootstrap、真实 GPU 样例与 submission drain 仍按后续切片实施 |

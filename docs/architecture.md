...
| vNext M10-A2b Catalog file load | 契约已冻结，实现随后续提交 | Core 有界 `readFile` + Asset `loadCatalogSnapshotFromManifestFile`（读盘→parse→CatalogSnapshot）。失败不发布。不实现 Handle/Lease、async IO、Cooker writer 或 GPU upload |

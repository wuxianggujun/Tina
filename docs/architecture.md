...
| vNext M10-A2a ContentHash digest | 契约已冻结，实现随后续提交 | Core 私有 XXH3-128 v1 adapter 计算版本化 `ContentHash`；公共 API 只有 Tina 类型；可选 `verifyCookedAssetContentHash` 校验 Cooked payload。A2a 不实现文件 IO、Handle/Lease、Catalog 磁盘加载、Cooker 或密码学签名 |

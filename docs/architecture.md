...
七个 C++23 target，以及 M10-A1 契约已冻结的 `tina_asset` 与可选的
...
| 路径资源系统 | 仍在使用 | M10-A0 已落地 Cooked wire schema、稳定 AssetId/ContentHash 值类型与只读校验；M10-A1 契约已冻结 owning `CatalogSnapshot`、AssetId binary search 与完整 DAG cycle 校验；AssetSystem、Catalog 文件 IO、Handle/Lease 和独立 GPU upload queue 尚未落地 |

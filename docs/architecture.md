...
| vNext M10-A2c Cooked object file load | 契约已冻结，实现随后续提交 | owning `CookedAssetFile`、单文件加载与 Catalog 根+AssetId 路径解析；parse + 可选 ContentHash + entry 对齐。不实现 Handle/Lease、async IO 或 GPU upload |

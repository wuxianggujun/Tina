...
`tina_platform_glfw` adapter、私有 `tina_render_bgfx` backend、Desktop bootstrap 和 M7-C1b/M7-C1c-a/C1c-b1
`tina_ui` tree/layout/committed-hit/point-query foundation。`tina_sample_null` 不依赖真实窗口和 GPU；`tina_sample_platform`
...
| vNext M7-C1b/C1c-a/C1c-b1 UI tree/layout/hit query foundation | 已完成 standalone `tina_ui` 树、布局、committed hit 与 point query 数据基础 | `Tina::UI` 只 PUBLIC 依赖 `Tina::Core` 与 `Tina::Platform`；在 generation `UINodeId`、`UIContext`、move-only `UIRootOwner`、结构 snapshot 与 route-result ABI 上，已实现 layout/dirty 类型、固定容量 PMR side array/queue/scratch、Flex-lite 非递归 Measure/Arrange、双缓冲 `UICommittedHitView`、structure/layout/hit 事务发布，以及无分配 `queryPointerHit()`；它按反向 paint order 选择 `Targetable` 且同时位于 world/clip 的首个目标，并返回 route index/revision/visited count。当前 changed frame 与 hit rebuild 仍全树扫描；listener token、Capture→Target→Bubble 路由、Focus/Capture/Modal、Button、paint snapshot/DisplayList、nested clip、bgfx UI pass 与 Runtime producer 后置 |
...
| 完整 vNext Runtime | 尚未完成 | GameStateStack/commands、worker、Pass Scheduler/RenderFramePacket、Scene/Asset/Audio、Runtime-integrated UI pipeline 与 submission drain 仍按后续切片实施；Desktop clear-only GPU 冒烟和 standalone `tina_ui` tree/layout/committed-hit/point-query foundation 不代表这些路径完成 |

...
| vNext M8-D0 Scene PerspectiveCamera3D/MeshRenderer3D extract | 已完成 Scene 3D 组件 + extract | `PerspectiveCamera3D`/`MeshRenderer3D` POD 存储、`extractRenderSceneFromWorld` 0/1 active perspective + fixture mesh/material → `setPerspectiveCamera`/`addMesh3D`；`tina_scene_tests` 34/34。无 AssetHandle/FrameResourceRef/Prefab；与 M9-B fixture / M11-E 产品路径正交 |
| vNext M11-E0–E5 product 3D StaticMesh + Unlit | 已完成最小产品竖切 | Cooked `StaticMeshPayload` + cube recipe、`GpuMeshId` upload/bind、UnlitBaseColor Material（solid + optional Texture2D）、Opaque3D textured unlit、`tina_sample_3d` 300 帧 JSON 与资源账本。**尚未** glTF/cgltf、Prefab 层级、PBR、Pass Scheduler；不单独满足 M12 Legacy 删除 |
...
| vNext M10-A2a ContentHash digest | 已完成 Core 私有 digest 切片 | Core PRIVATE 链接 xxHash；`digestContentHashV1` 计算 XXH3-128 seed=0 的 16 字节 little-endian `ContentHash`；`verifyCookedAssetContentHash` 校验 Cooked payload。公共头零 `XXH*` token。A2a 不实现文件 IO、Handle/Lease、Catalog 磁盘加载、Cooker 或密码学签名 |
| vNext M10-A2b Catalog file load | 已完成有界文件加载切片 | Core 有界 `readFile` + Asset `loadCatalogSnapshotFromManifestFile`（读盘→parse→CatalogSnapshot）。失败不发布。不实现 Handle/Lease、async IO、Cooker writer 或 GPU upload |
| vNext M10-A2c Cooked object file load | 已完成 owning 文件加载切片 | owning `CookedAssetFile`、单文件加载与 Catalog 根+AssetId 路径解析；parse + 可选 ContentHash + entry 对齐。不实现 Handle/Lease、async IO 或 GPU upload |
...
| vNext M10-A2h catalog validate CLI | 已完成工具切片 | `tina_catalog_validate` 同步打开/校验磁盘包并输出 JSON；不引入 registry/Handle/Lease/async |
| vNext M10-A2i package summary | 已完成诊断摘要切片 | `buildCatalogPackageSummary` + CLI `--list-entries`；不触盘、不引入 Handle/Lease |
...
| vNext M10-A2n package load polish | 已完成覆盖与 CLI 加载切片 | 空请求加载全部 + CLI `--load-assets`/`--asset-id`；不引入 Handle/Lease |
| vNext M10-A2o pipeline e2e fixture | 已完成端到端测试夹具切片 | 共享 `CatalogPackageTestSupport` + open→plan→load→validate→summary 流水线测试。不引入 Handle/Lease |
| vNext M10-A2p plan byte total | 已完成计划字节汇总切片 | `totalCookedFileBytes` + CLI `plannedCookedFileBytes`；checked overflow。不引入 Handle/Lease |
| vNext M10-A2q batch byte budget | 已完成加载预算门禁切片 | `maxTotalCookedFileBytes` 在读盘前拒绝超预算 plan；不引入 Handle/Lease |
| vNext M10-A3 AssetStore Handle/Lease | 已完成 CPU 生命周期首切片 | `AssetStore` 发布 Ready cooked payload；`AssetHandle` 弱查询；`AssetLease` 强保活；unload 延迟到末 lease。无 Loading/GPU UploadTicket/retirement ledger |
| vNext M10-A4 AssetSystem sync facade | 已完成 Catalog 绑定同步加载切片 | `AssetSystem`：bindCatalog、依赖序同步 load、AssetId→Handle 去重、失败仅回滚本次 publish。无异步状态机/GPU |
| vNext M10-A5 deferred request/pump | 已完成主线程 deferred 切片 | `request` 入队 Queued；`pump(budget)` 推进 Loading→Ready/Failed；有界队列。仍 owner-thread 同步 IO，无 Task worker/GPU |
| vNext M10-A6 bounded IO Task + asset pump | 已完成 Task/Asset 异步读盘首切片 | `BoundedTaskSystem` IO+Main；AssetSystem 可选注入 taskSystem；pump 调度 IO 并 drain Main complete。无 CPU pool/TaskGroup/GPU |
| vNext M10-A7 Desktop Task + Null UploadTicket | 已完成生产默认 Task 与 Upload SPI | Desktop 默认 BoundedTaskSystem；`NullUploadLedger` 拥有 staging 直至 retire。无 bgfx fence/ReadyGpu |
| vNext M10-A8 ReadyGpu via Null upload | 已完成 CPU→GPU 逻辑状态切片 | ReadyCpu→UploadQueued→ReadyGpu；`AssetGpuUploadCoordinator` 接 Null ledger。无 bgfx 资源创建 |
...
| vNext M10-A13 retirement ledger | 已完成逻辑退役账本切片 | unload 取消 pending upload 并 retire staging；sample 支持 `--catalog=`。无 bgfx fence |
| vNext M10-A14 recipe catalog cook | 已完成 recipe cooker 切片 | `CatalogCook` + `tina_assetc --recipe`；payload 文件依赖边。无 cgltf |
| vNext M10-A15 Texture2D/Sprite payload | 已完成 2D payload schema 切片 | Texture2D Rgba8Unorm + Sprite UV/pivot/PPU v1。无 bgfx 采样绘制 |
| vNext M10-A16 typed 2D product path | 已完成默认 2D typed 闭环 | assetc/sample 默认 Texture2D+Sprite 并解析 payload。无 bgfx 绘制 |
| vNext M10-A17 AssetSystem catalog helpers | 已完成打开/按类型查找切片 | openAndBindCatalog + kind 查找 + typed views。无 bgfx 绘制 |
...
| vNext M10-A22 bgfx Sprite2D texture sample | 已完成采样管线切片 | Sprite2D shader 采样 `s_tex`，默认白纹理。Catalog 像素上传后置 |
...
| vNext M10-A32 Catalog TileMap bgfx sample | 已完成可见烟测 | `tina_sample_2d_tilemap_bgfx`：cook/load Texture2D+Tileset+TileMap，atlas 上传，emit 11 tile + 角色 |
...
| vNext M10-A37 sample walk + HUD Button | 已完成 | 键位绑定 + 落地后右走撞墙 JSON 门禁 + HUD Button action 接线 |
| vNext M10-A38 sample catalog recipe file | 已完成 | `sample_2d.recipe` 磁盘加载 cook；JSON `catalogFromRecipeFile`。完整 cooker CLI 后置 |
...
| vNext M11-A6 TileMap→Physics2D bridge | 已完成直接门禁 | Asset `TileMapPhysicsSync` 收集 MaterialSolid 并调用 A5；`Tina::Asset` 在 PHYSICS2D 图 PUBLIC 链接 Physics2D。`tina_sample_2d` product-2d 图已消费 |

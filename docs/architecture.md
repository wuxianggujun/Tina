...
- `TinaEditor.exe`（target `tina_editor_desktop`）：独立 `Tina::EditorApp` 的 2D/3D canonical document、
  独立 path/baseline/dirty workspace session、Inspector、Runtime preview 与 GPU viewport；

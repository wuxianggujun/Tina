...
| `Tina::Scene::World` | M8-A 已实现 standalone owner；M8-C0 组件存储 | Scene public；尚未接入 Phase Context | move-only、owner-thread 读写、Create 时固定 entity/遍历/scratch storage；可选 Camera2D/SpriteRenderer2D；`liveEntities()`；析构归还 supplied PMR | invalid capacity/owner thread/corrupt hierarchy/InvalidComponent |
...
| `extractRenderSceneFromWorld` | M8-C0 已实现；M8-C2 UV 转发 | Scene public free function | World → phase-local `RenderSceneWriter`；先 updateWorldTransforms；0/1 active Camera2D；fixture sprite 路径（含可选 UV rect） | MultipleActiveCameras / InvalidComponent / UnresolvedSprite / Render projection errors |

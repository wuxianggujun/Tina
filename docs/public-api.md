...
| `readFile` / `ReadFileConfig` | M10-A2b 已实现 | Core public | UTF-8 路径有界同步读文件到 owning PMR bytes | InvalidArgument / NotFound / PermissionDenied / Io / CapacityExceeded / OutOfMemory |
...
| `loadCookedAssetsFromPlan` | M10-A2l 已实现 | Asset module public | 按既有 plan 同步加载 owning CookedAssetFile；失败不发布部分批 | NotFound / CatalogEntryMismatch / Io / ContentHashMismatch |
| `loadCookedAssetsFromPackage` / `CatalogPackageCookedLoad` | M10-A2m 已实现 | Asset module public | open+plan+load 一站式；返回 owning Catalog + assets；失败全不发布 | open/plan/load 任一步错误 |
| `tina_catalog_validate --load-assets` | M10-A2n 已实现 | host tool | CLI 一站式加载；JSON `loadedAssets`/`loaded`；可与 `--asset-id` 组合 | 同 package load 错误 |
| `totalCookedFileBytes` | M10-A2p 已实现 | Asset module public | 汇总 plan 的 cookedFileBytes；checked overflow | CapacityExceeded |
...
| `createBoundedTaskSystem` | M10-A6 已实现 | Task module public factory | 有界 IO workers + Main completion queue | QueueFull / TaskSystemStopped / InvalidArgument |
| `ITaskSystem::scheduleIo` / `postMain` / `pumpMain` | M10-A6 已实现 | Task SPI | IO 调度与主线程 completion 抽取 | QueueFull / NotSupported（Disabled） |
...
| `AssetRetirementLedger` | M10-A13 已实现 | Asset module public | DestroyQueued→Retiring→Released 诊断账本 | 无 |
| `AssetGpuUploadCoordinator::cancelUpload` | M10-A13 已实现 | Asset module public | unload 时 retire 未完成 UploadTicket | ledger/store 错误 |
| `tina_sample_asset --catalog=` | M10-A13 已实现 | sample | 加载外部 Catalog（可接 tina_assetc） | sample JSON |
| `cookCatalogPackage` / `cookAndPublishCatalogPackage` | M10-A14 已实现 | Asset module public | 内存 cook + 磁盘 publish | Invalid / wire writer 错误 |
| `loadCatalogCookRecipeFile` / `parseCatalogCookRecipe` | M10-A14 已实现 | Asset module public | 行式 recipe → CatalogCookRequest | Invalid / Io |
| `tina_assetc --recipe` | M10-A14 已实现 | host tool | recipe 驱动 fixture/cook 包 | 同 cook/publish/open |
| `writeTexture2DPayloadBytes` / `parseTexture2DPayload` / `writeCookedTexture2DAsset` | M10-A15 已实现 | AssetFormat module public | Texture2D payload v1 (Rgba8Unorm) | InvalidLayout / Unsupported* |
| `writeSpritePayloadBytes` / `parseSpritePayload` / `writeCookedSpriteAsset` | M10-A15 已实现 | AssetFormat module public | Sprite UV/pivot/PPU + Texture2D 依赖 | InvalidLayout / InvalidIdentity |
| `tina_assetc` default typed2d fixture | M10-A16 已实现 | host tool | 默认输出 Texture2D 2x2 + Sprite；`--legacy-text` 兼容旧文本包 | 同 cook/publish |
| `tina_sample_asset` typed parse | M10-A16 已实现 | sample | 加载后 parse Texture2D/Sprite payload 并输出尺寸/PPU | sample JSON |
...
| recipe `texture2d` / `sprite` lines | M10-A19 已实现 | CatalogCook / assetc | 内联 RGBA 像素与 Sprite 参数 | Invalid / payload 错误 |
| `RenderSprite2DInput/Item` UV `u0,v0,u1,v1` | M10-A20 已实现 | Render module public | Sprite 子矩形 UV（默认全图） | InvalidRenderSceneInput |
| `makeSpriteRenderInput` | M10-A20 已实现 | Asset module public | typed Sprite(+Texture) → RenderSprite2DInput | typed parse / size 错误 |
| Asset→RenderScene CPU pipeline (tests) | M10-A21 已实现 | test gate | cook/load/typed/render commit 闭环 | 见测试 |
...
| `ITaskSystem::scheduleCpu` / `TaskSystemCreateParams::cpuWorkerCount` | M10-A25 已实现 | Task module public | 有界 CPU worker 域 | NotSupported / QueueFull |
| `TaskGroup` | M10-A25 已实现 | Task module public | CPU 域结构化 add/waitIdle | WaitTimeout / TaskSystemStopped |
| `writeTilesetPayloadBytes` / `parseTilesetPayload` / `writeCookedTilesetAsset` | M10-A26 已实现 | AssetFormat module public | Tileset UV/material + Texture2D 依赖 | InvalidLayout / Unsupported* |
| `writeTileMapPayloadBytes` / `parseTileMapPayload` / `writeCookedTileMapAsset` | M10-A26 已实现 | AssetFormat module public | 单层 TileMap cells + Tileset 依赖 | InvalidLayout / Unsupported* |
...
| `Tina::Scene::World` | M8-A 已实现 standalone owner；M8-C0/M8-D0 组件存储 | Scene public；尚未接入 Phase Context | move-only、owner-thread 读写、Create 时固定 entity/遍历/scratch storage；可选 Camera2D/SpriteRenderer2D/PerspectiveCamera3D/MeshRenderer3D；`liveEntities()`；析构归还 supplied PMR | invalid capacity/owner thread/corrupt hierarchy/InvalidComponent |
| `Tina::Scene::Camera2D` / `SpriteRenderer2D` | M8-C0 已实现；M8-C2 可选 UV | Scene public POD | 投影/viewport/snap/active；fixtureSpriteKey + size/pivot/optional UvRect/color/sort/flip/visible；无 GPU handle | InvalidComponent |
| `Tina::Scene::PerspectiveCamera3D` / `MeshRenderer3D` | M8-D0 已实现 | Scene public POD | FOV/near/far/viewport/active；fixture meshKey/materialKey + submesh/localBounds/baseColor/doubleSided/visible；无 GPU handle | InvalidComponent |
| `Tina::Scene::instantiatePrefab` | M11-E8 已实现 | Scene public free function | PrefabPayloadView → World 实体层级 + 可选 MeshRenderer3D（fixture mesh binding）；失败全回滚 | InvalidComponent / CorruptHierarchy / capacity |
| `Tina::Asset::cookGltfFileToCatalogRequest` | M11-E7 已实现 | Asset module public | cgltf 解析 glTF/GLB → CatalogCookRequest（首 mesh/TRIANGLES → StaticMesh+Material+Prefab） | CatalogFileLoadFailed / InvalidCatalogConfig |
| `Tina::Asset::parsePrefabFromCooked` | M11-E6b 已实现 | Asset module public | Cooked Prefab → OwnedPrefabPayload（nodes 自有） | CatalogEntryMismatch / AssetFormat layout |
| `extractRenderSceneFromWorld` | M8-C0/C2 2D；M8-D0 3D | Scene public free function | World → phase-local `RenderSceneWriter`；先 updateWorldTransforms；0/1 active Camera2D 与 0/1 active PerspectiveCamera3D（轨道独立）；fixture sprite/mesh 路径（含可选 UV rect） | MultipleActiveCameras / InvalidComponent / UnresolvedSprite / UnresolvedMesh / Render projection errors |

...
| `Tina::Scene::Camera2D` / `SpriteRenderer2D` | M8-C0 已实现 | Scene public POD | 投影/viewport/snap/active；fixtureSpriteKey + size/pivot/color/sort/flip/visible；无 GPU handle | InvalidComponent |
| `extractRenderSceneFromWorld` | M8-C0 已实现 | Scene public free function | World → phase-local `RenderSceneWriter`；先 updateWorldTransforms；0/1 active Camera2D；fixture sprite 路径 | MultipleActiveCameras / InvalidComponent / UnresolvedSprite / Render projection errors |

...
// Composes a cooked SkinnedMesh's own bind pose into a Render skinned palette.
// Deriving it from the skeleton rather than writing identity matrices keeps this
// correct whatever convention the cook uses for inverseBindMatrices.
[[nodiscard]] Tina::Core::Result<std::vector<float>> composeBindPosePalette(
    const Tina::Asset::CookedAssetFile& meshFile){
    auto meshView = Tina::Asset::parseSkinnedMeshFromCooked(meshFile);
    if (!meshView) {
        return Tina::Core::failure(std::move(meshView.error()));
    }
    auto skeleton = Tina::Animation3D::Skeleton3D::Create(*meshView);
    if (!skeleton) {
        return Tina::Core::failure(std::move(skeleton.error()));
    }
    auto pose = Tina::Animation3D::Pose3D::Create(skeleton->jointCount());
    if (!pose) {
        return Tina::Core::failure(std::move(pose.error()));
    }
    if (auto status = skeleton->writeBindPose(*pose); !status) {
    ...
    try {
        ...
        return Tina::Core::failure(Tina::Core::CoreErrorCode::OutOfMemory,
                                   "editor World3D preview skinned palette allocation failed");
    }
    if (auto status = skeleton->composeSkinningMatrices(*pose, palette); !status) {
        return Tina::Core::failure(std::move(status.error()));
    }
    return palette;
}
...
auto EditorWorkspaceState::resolvePreviewSkinnedPose(
    ...
    // An empty span fails extraction closed rather than uploading a garbage palette,
    // which is what an entity whose skeleton did not compose should do.
    return pose == self.preview3DSkinnedPoses_.end() ? std::span<const float>{}
                                                     : std::span<const float>{pose->palette};
}
...
auto EditorWorkspaceState::validateWorld3DRuntimePreview() -> Tina::Core::Status{
    ...
    std::vector<World3DPreviewSkinnedPose> skinnedPoses;
    try {
    ...
    for (u32 index = 0; index < nodeStorage.size(); ++index) {
        ...
        if (node.nodeKind == Tina::AssetFormat::PrefabNodeKind::Mesh3D) {
            ...
            if (meshFile == nullptr) {
                return Tina::Core::failure(
                    Tina::Core::CoreErrorCode::Internal,
                    "editor World3D preview SkinnedMesh3D payload is unavailable");
            }
            auto palette = composeBindPosePalette(*meshFile);
            if (!palette) {
                return Tina::Core::failure(std::move(palette.error()));
            }
            try {
                skinnedPoses.push_back({.entity = entity, .palette = std::move(*palette)});
            } catch (const std::bad_alloc&) {
                return Tina::Core::failure(
                    Tina::Core::CoreErrorCode::OutOfMemory,
                    "editor World3D preview skinned pose allocation failed");
            }
        }
    }

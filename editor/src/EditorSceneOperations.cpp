...
try
{
    ...
    for (const auto& node : storage) {
        auto desc = AssetFormat::prefabNodeDescFromView(node);
...
try
{
    ...
    for (Core::usize index = 0; index < storage.size(); ++index) {
        ...
        auto duplicate = AssetFormat::prefabNodeDescFromView(storage[index]);
...
try
{
    ...
    for (Core::usize index = 0; index < storage.size(); ++index) {
        ...
        nodes.push_back({
            .node = AssetFormat::prefabNodeDescFromView(storage[index]),
...
try
{
    ...
    while (!pending.empty()) {
        ...
        hierarchy.push_back({
            .node = AssetFormat::prefabNodeDescFromView(node),

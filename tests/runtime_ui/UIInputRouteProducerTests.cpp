                                        ...
                                        std::pmr::memory_resource& resource = *std::pmr::get_default_resource())
{
    ...
    auto panel = tree.context->rootBuilder().createElement(tree.root.rootNodeId(), UI::makePanelElement());
    EXPECT_TRUE(panel.has_value()) << (panel ? "" : panel.error().message);
    if (!panel)
    {
        return tree;
    }
    auto target = tree.context->rootBuilder().createElement(*panel, UI::makeButtonElement());
...
[[nodiscard]] RouteTree createTextEditRouteTree(Platform::WindowId window)
{
    ...
    auto textEdit = tree.updater.createElement(tree.panel, UI::makeTextEditElement());
...
[[nodiscard]] DropdownRouteTree createDropdownRouteTree(Platform::WindowId window)
{
    ...
    auto before = tree.updater.createElement(tree.root.rootNodeId(), UI::makeButtonElement());
    auto dropdown = tree.updater.createElement(tree.root.rootNodeId(), UI::makeDropdownElement());
    ...
    auto popup = tree.updater.createElement(*dropdown, UI::makePopupElement());
    EXPECT_TRUE(popup.has_value()) << (popup ? "" : popup.error().message);
    if (!popup)
    {
        return tree;
    }
    auto firstItem = tree.updater.createElement(*popup, UI::makeDropdownItemElement());
    auto secondItem = tree.updater.createElement(*popup, UI::makeDropdownItemElement());
    auto after = tree.updater.createElement(tree.root.rootNodeId(), UI::makeButtonElement());
...
[[nodiscard]] CollectionRouteTree createCollectionRouteTree(Platform::WindowId window)
{
    ...
    auto listView = tree.updater.createElement(
        tree.root.rootNodeId(), UI::makeListViewElement({.materializedItemCapacity = 6}));
    auto treeView = tree.updater.createElement(
        tree.root.rootNodeId(), UI::makeTreeViewElement({.materializedItemCapacity = 6}));
    auto other = tree.updater.createElement(tree.root.rootNodeId(), UI::makeButtonElement());

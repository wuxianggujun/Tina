...
TEST(TileChunkExtractionTests, VisibleChunksSkipEmptyAndRespectCamera)
{
    ...
    auto count = extractVisibleTileChunks(
        map, TileChunkCameraQuery{.centerX = 2.0f, .centerY = 2.0f, .halfWidth = 3.0f, .halfHeight = 3.0f}, chunks);
    ...
    count = extractVisibleTileChunks(
        map, TileChunkCameraQuery{.centerX = 3.5f, .centerY = 3.5f, .halfWidth = 0.4f, .halfHeight = 0.4f}, chunks);
    ASSERT_TRUE(count.has_value());
    EXPECT_EQ(*count, 0U);
}
...
TEST(TileChunkExtractionTests, CollectChunkCellsAndGridCollision)
{
    ...
    auto n = collectChunkNonEmptyCells(map, TileMapChunkCoord{.chunkX = 0, .chunkY = 0}, cells);

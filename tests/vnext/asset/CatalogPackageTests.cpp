...
TEST(CatalogPackageTests, OpensValidPackageWithValidation)
{
    TrackingMemoryResource resource;
    constexpr Core::u8 Seed = 0x31U;
    constexpr std::array<std::byte, 4> Payload{std::byte{0x10}, std::byte{0x20}, std::byte{0x30}, std::byte{0x40}};
    const auto digest = Core::digestContentHashV1(Payload);
    ...
    {
        auto snapshot = openCatalogPackage(toUtf8(catalogRoot), config);
        ASSERT_TRUE(snapshot.has_value()) << snapshot.error().message;
        EXPECT_EQ(snapshot->entryCount(), 1U);
        ASSERT_TRUE(snapshot->find(assetId));
    }
...
TEST(CatalogPackageTests, ValidationFailureDoesNotPublishSnapshot)
{
    TrackingMemoryResource resource;
    constexpr Core::u8 Seed = 0x32U;
    constexpr std::array<std::byte, 4> Payload{std::byte{0x10}, std::byte{0x20}, std::byte{0x30}, std::byte{0x40}};
    const auto digest = Core::digestContentHashV1(Payload);
    ASSERT_TRUE(digest.has_value());
    ...
    const auto snapshot = openCatalogPackage(toUtf8(catalogRoot), config);
    ASSERT_FALSE(snapshot.has_value());
    EXPECT_EQ(snapshot.error().code, Core::CoreErrorCode::NotFound);
    EXPECT_EQ(resource.outstandingAllocations(), 0U);
...
TEST(CatalogPackageTests, RejectsUnsafeManifestRelativePath)
{
    TrackingMemoryResource resource;
    CatalogPackageOpenConfig config{
        .manifest =
    ...
    ASSERT_FALSE(snapshot.has_value());
    EXPECT_EQ(snapshot.error().code, AssetErrorCode::InvalidCatalogConfig);
}

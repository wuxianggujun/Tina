...
inline std::vector<std::byte> readAudioFixture(std::string_view name)
{
    ...
    std::vector<std::byte> bytes(static_cast<Tina::Core::usize>(file.tellg()));
    file.seekg(0);
...
inline std::vector<std::byte> makePcmWav(std::span<const float> samples,
                                       Tina::Core::u16 channels = 1,
                                       Tina::Core::u16 bits = 16,
                                       bool floatingPoint = false,
                                       Tina::Core::u32 sampleRate = 8000)
{
    std::vector<std::byte> bytes;
    const auto append = [&](Tina::Core::u64 value, Tina::Core::usize count) {
        for (Tina::Core::usize index = 0; index < count; ++index) {
    ...
    const auto dataBytes = static_cast<Tina::Core::u32>(samples.size() * (bits / 8));
    ...
    for (float sample : samples) {
        if (floatingPoint) {
            append(bits == 32 ? std::bit_cast<Tina::Core::u32>(sample)
                              : std::bit_cast<Tina::Core::u64>(static_cast<double>(sample)), bits / 8);
        } else if (bits == 8) {
            append(static_cast<Tina::Core::u8>((sample + 1.0F) * 128.0F), 1);
        } else {
            append(static_cast<Tina::Core::u64>(static_cast<Tina::Core::i64>(
                static_cast<double>(sample) * static_cast<double>(Tina::Core::u64{1} << (bits - 1)))), bits / 8);
        }
    }
